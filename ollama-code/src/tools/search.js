...
class SearchTool extends types_js_1.BaseTool {
    constructor() {
        super(...arguments);
        this.metadata = {
            name: 'search',
    ...
    async execute(parameters, context) {
        ...
        try {
            ...
            const { query, path: searchPath = '.', type = 'content', filePattern, caseSensitive = false, useRegex = false, contextLines = 2, maxResults = 100, excludePatterns = (0, file_patterns_js_1.getDefaultExcludePatterns)(), respectGitIgnore = true } = parameters;
            const resolvedPath = path_1.default.resolve(context.workingDirectory, searchPath);
            // Security check
            ...
            logger_js_1.logger.error(`Search tool error: ${error}`);
            return {
                success: false,
    ...
    async performSearch(searchPath, options) {
        ...
        if (options.respectGitIgnore && options.projectRoot) {
            try {
                gitIgnoreParser = (0, gitignore_parser_js_1.getGitIgnoreParser)(options.projectRoot);
            }
            catch (error) {
                logger_js_1.logger.warn('Failed to load .gitignore parser for searching', error);
            }
        }
        const excludeRegexes = options.excludePatterns.map(pattern => (0, file_patterns_js_1.globToRegex)(pattern));
        ...
        const searchDirectory = async (dirPath) => {
            ...
            for (const entry of entries) {
                ...
                const relativePath = path_1.default.relative(searchPath, fullPath);
    ...
    async searchFileContent(filePath, relativePath, options, matches) {
        try {
            ...
            logger_js_1.logger.debug(`Skipping file ${filePath}: ${error}`);
        }
    }
