...
class ToolOrchestrator extends events_1.EventEmitter {
    ...
    async executeTool(toolName, parameters, context) {
        const tool = registry_js_1.toolRegistry.get(toolName);
        if (!tool) {
        ...
        if (this.config.enableCaching && this.executionCache.has(cacheKey)) {
            ...
            logger_js_1.logger.debug(`Using cached result for ${toolName}`);
            return cachedResult;
        }
        const execution = {
     ...
     */
    async executeOrchestration(plan, context) {
        ...
        logger_js_1.logger.info(`Starting orchestrated execution of ${queue.length} tools (max ${maxConcurrent} concurrent)`);
        while (queue.length > 0 || inProgress.size > 0) {
            // Find ready executions (dependencies satisfied)
            const readyExecutions = queue.filter(execution => execution.dependencies.every(dep => completed.has(dep)));
            ...
            for (const execution of toStart) {
                ...
                this.executeTool(execution.toolName, execution.parameters, context)
                    ...
                    logger_js_1.logger.debug(`Completed execution ${execution.id} (${execution.toolName})`);
                })
                    .catch(error => {
                    const errorResult = {
                        success: false,
                    ...
                    completed.add(execution.id);
                    inProgress.delete(execution.id);
                    logger_js_1.logger.error(`Failed execution ${execution.id} (${execution.toolName}): ${error.message}`);
                });
            }
            ...
            if (toStart.length === 0 && inProgress.size === 0 && queue.length > 0) {
                logger_js_1.logger.error('Deadlock detected in orchestration plan - circular dependencies?');
                break;
            }
        }
        return results;
    ...
    validateDependencies(plan) {
        ...
        const hasCycle = (execId) => {
            if (recursionStack.has(execId))
                return true;
            if (visited.has(execId))
                return false;
            visited.add(execId);
