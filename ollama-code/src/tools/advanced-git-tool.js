...
exports.AdvancedGitTool = void 0;
const types_js_1 = require("./types.js");
const child_process_1 = require("child_process");
const path = __importStar(require("path"));
class AdvancedGitTool extends types_js_1.BaseTool {
    constructor() {
        super(...arguments);
        this.metadata = {
            name: 'advanced-git',
    ...
    async analyzeRepository(context, options) {
    ...
    analyzeRepoStructure(workingDir) {
        try {
    ...
    getRepoStatistics(workingDir) {
        try {
            ...
            const linesOfCode = this.executeGitCommand('git ls-files | xargs wc -l 2>/dev/null | tail -1 | awk \'{print $1}\'', workingDir).trim();
            return {
                totalCommits,
                totalBranches,
                totalContributors,
    ...
    assessRepoHealth(workingDir) {
        ...
        try {
            ...
            const largeFiles = this.executeGitCommand('git ls-files | xargs ls -la 2>/dev/null | awk \'$5 > 1048576 {print $9}\' | head -5', workingDir).split('\n').filter(f => f.trim());
    ...
    async getCommitHistory(context, options) {
        try {
    ...
    async analyzeBranches(context, options) {
        try {
            ...
            const branchInfo = localBranches.map(branch => {
                try {
                    const lastCommit = this.executeGitCommand(`git log -1 --format="%h %s (%cr)" ${branch}`, context.workingDirectory).trim();
                    const commitCount = parseInt(this.executeGitCommand(`git rev-list --count ${branch}`, context.workingDirectory).trim());
                    return {
                        name: branch,
    ...
    async analyzeContributors(context, options) {
        try {
            ...
            const sinceFlag = this.getTimeframeFlag(timeframe);
            const contributorStats = this.executeGitCommand(`git shortlog -sn ${sinceFlag}`, context.workingDirectory);
    ...
    analyzeDevelopmentPatterns(workingDir) {
        try {
            const hourlyActivity = this.executeGitCommand('git log --format="%ad" --date=format:"%H" | sort | uniq -c | sort -nr', workingDir);
            const dayActivity = this.executeGitCommand('git log --format="%ad" --date=format:"%u" | sort | uniq -c | sort -nr', workingDir);
            return {
    ...
    analyzeCodeEvolution(workingDir) {
        try {
            const fileChanges = this.executeGitCommand('git log --format="" --name-only | sort | uniq -c | sort -nr | head -10', workingDir);
            return {
    ...
    analyzeCollaboration(workingDir) {
        try {
            const authorFiles = this.executeGitCommand('git log --format="%an" --name-only | grep -v "^$" | sort | uniq -c | sort -nr | head -10', workingDir);
            return {
    ...
    generateRecommendations(workingDir) {
...
exports.AdvancedGitTool = AdvancedGitTool;
//# sourceMappingURL=advanced-git-tool.js.map