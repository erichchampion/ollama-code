...
exports.getGitIgnoreParser = getGitIgnoreParser;
...
class GitIgnoreParser {
    ...
    loadGitIgnoreFiles() {
        ...
        while (depth < MAX_DEPTH) {
            ...
            if (fs.existsSync(gitignorePath)) {
                try {
                    ...
                    logger_js_1.logger.warn(`Could not read .gitignore at ${gitignorePath}`, { error });
                }
            }
        ...
        if (depth >= MAX_DEPTH) {
            logger_js_1.logger.warn(`Reached maximum depth (${MAX_DEPTH}) while searching for .gitignore files`);
        }
    ...
    loadGlobalGitIgnore() {
        try {
            ...
            for (const filename of globalGitignoreNames) {
                ...
                if (fs.existsSync(globalGitIgnorePath)) {
                    try {
                        ...
                        this.rules.push(...rules);
                        logger_js_1.logger.debug(`Loaded global gitignore from ${globalGitIgnorePath}`);
                        break; // Only load the first found global gitignore
                    }
                    catch (error) {
                        logger_js_1.logger.debug(`Could not read global gitignore at ${globalGitIgnorePath}`, { error });
                    }
                }
            }
        }
            ...
            logger_js_1.logger.debug('Error loading global gitignore', { error });
        }
    }
    /**
 ...
 */
function getGitIgnoreParser(projectRoot) {
