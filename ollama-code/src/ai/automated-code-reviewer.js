...
exports.AutomatedCodeReviewer = void 0;
const logger_js_1 = require("../utils/logger.js");
const performance_js_1 = require("../config/performance.js");
const architectural_analyzer_js_1 = require("./architectural-analyzer.js");
const complexity_calculator_js_1 = require("../utils/complexity-calculator.js");
const language_detector_js_1 = require("../utils/language-detector.js");
const fs = __importStar(require("fs/promises"));
const path = __importStar(require("path"));
class AutomatedCodeReviewer {
    constructor() {
        this.config = (0, performance_js_1.getPerformanceConfig)();
        this.architecturalAnalyzer = new architectural_analyzer_js_1.ArchitecturalAnalyzer();
    }
    /**
    ...
    async reviewCode(request) {
        ...
        try {
            logger_js_1.logger.info('Starting automated code review', {
                files: request.files.length,
                context: request.context?.pullRequestId
            });
            // Analyze architecture and context
            const architecturalAnalysis = await this.architecturalAnalyzer.analyzeArchitecture(request.files.map(f => ({ path: f.path, content: f.content, type: f.language || 'unknown' })));
            // Review each file
            const fileReviews = [];
            ...
            const recommendations = this.generateRecommendations(fileReviews, architecturalAnalysis, request.context);
            // Calculate metrics
            const metrics = this.calculateReviewMetrics(fileReviews, performance.now() - startTime);
            const result = {
                summary,
                files: fileReviews,
                overallAssessment,
            ...
            logger_js_1.logger.info(`Code review completed in ${duration.toFixed(2)}ms`, {
            ...
            logger_js_1.logger.error('Code review failed:', error);
            throw error;
        }
    }
    async reviewFile(file, options, architecturalContext) {
        const startTime = performance.now();
        try {
            ...
            const limitedIssues = options?.maxIssuesPerFile
                ...
                : filteredIssues;
            const duration = performance.now() - startTime;
            logger_js_1.logger.debug(`File review completed: ${file.path}`, {
            ...
            logger_js_1.logger.error(`Failed to review file: ${file.path}`, error);
            throw error;
        }
    }
    async calculateFileMetrics(file) {
        ...
        const complexityResult = (0, complexity_calculator_js_1.calculateCyclomaticComplexity)(file.content);
        return {
            linesOfCode,
    ...
    async analyzeCodeQuality(file, metrics) {
        const issues = [];
    ...
    async analyzeMaintainability(file, metrics) {
        const issues = [];
    ...
    async analyzeDocumentation(file) {
        const issues = [];
    ...
    async analyzeArchitecture(file, context) {
        const issues = [];
    ...
    async analyzeBestPractices(file) {
        const issues = [];
    ...
    findPositivePoints(file, metrics) {
        const points = [];
    ...
    generateFileSuggestions(file, issues, metrics) {
        const suggestions = [];
    ...
    generateOverallAssessment(fileReviews, architecturalAnalysis) {
        const totalIssues = fileReviews.reduce((sum, review) => sum + review.issues.length, 0);
        const criticalIssues = fileReviews.reduce((sum, review) => sum + review.issues.filter(i => i.severity === 'critical').length, 0);
        const majorIssues = fileReviews.reduce((sum, review) => sum + review.issues.filter(i => i.severity === 'major').length, 0);
        // Calculate quality scores
        const avgMaintainability = fileReviews.reduce((sum, review) => sum + review.metrics.maintainabilityIndex, 0) / fileReviews.length / 100;
        const avgComplexity = fileReviews.reduce((sum, review) => sum + Math.min(review.metrics.cyclomaticComplexity / 20, 1), 0) / fileReviews.length;
    ...
    createReviewSummary(fileReviews, overallAssessment) {
        const totalIssues = fileReviews.reduce((sum, review) => sum + review.issues.length, 0);
        const criticalIssues = fileReviews.reduce((sum, review) => sum + review.issues.filter(i => i.severity === 'critical').length, 0);
        const majorIssues = fileReviews.reduce((sum, review) => sum + review.issues.filter(i => i.severity === 'major').length, 0);
        const minorIssues = fileReviews.reduce((sum, review) => sum + review.issues.filter(i => i.severity === 'minor').length, 0);
        ...
        const reviewStatus = overallAssessment.recommendedAction === 'approve' ? 'approve' :
    ...
    generateRecommendations(fileReviews, architecturalAnalysis, context) {
    ...
    calculateReviewMetrics(fileReviews, reviewDuration) {
        const totalLines = fileReviews.reduce((sum, review) => sum + review.metrics.linesOfCode, 0);
        const totalIssues = fileReviews.reduce((sum, review) => sum + review.issues.length, 0);
        const avgQualityScore = fileReviews.reduce((sum, review) => sum + review.metrics.maintainabilityIndex, 0) / fileReviews.length;
        // Calculate complexity distribution
        const complexityDistribution = fileReviews.reduce((dist, review) => {
        ...
        return {
            ...
            reviewDuration,
            coverageAnalysis: {
                functionsReviewed: fileReviews.reduce((sum, review) => sum + this.extractMethods(review.filePath, review.language).length, 0),
                classesReviewed: fileReviews.reduce((sum, review) => sum + this.extractClasses(review.filePath, review.language).length, 0),
    ...
    detectLanguage(filePath) {
        return (0, language_detector_js_1.detectLanguageFromPath)(filePath) || 'unknown';
    }
    ...
    findMagicNumbers(content) {
        ...
        while ((match = numberRegex.exec(content)) !== null) {
            ...
            if (['0', '1', '2', '100', '0.0', '1.0'].includes(value))
                continue;
    ...
    extractPublicFunctions(content) {
        const functions = [];
    ...
    async hasCorrespondingTests(filePath) {
...
exports.AutomatedCodeReviewer = AutomatedCodeReviewer;
//# sourceMappingURL=automated-code-reviewer.js.map