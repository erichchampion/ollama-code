...
exports.GitChangeTracker = void 0;
...
class GitChangeTracker {
    constructor(config = {}) {
        ...
        const performanceConfig = (0, performance_js_1.getPerformanceConfig)();
        this.config = {
    ...
    async initialize() {
        try {
            ...
            logger_js_1.logger.info('Git change tracker initialized', {
                mode: this.config.trackingMode,
                repo: this.config.repositoryPath
            });
        }
        catch (error) {
            logger_js_1.logger.error('Failed to initialize git change tracker', { error });
            throw error;
        }
    }
    /**
    ...
    async getChangesSinceLastUpdate() {
        try {
            ...
            logger_js_1.logger.error('Failed to get git changes', { error });
            return [];
        }
    }
    /**
     ...
     */
    async getChangesSinceCommit(sinceCommit) {
        const { stdout } = await execAsync(`git diff --name-status ${sinceCommit}..HEAD`, { cwd: this.config.repositoryPath });
        const changes = [];
        if (stdout.trim()) {
     ...
     */
    async getChangesSinceDate(sinceDate) {
        const dateStr = sinceDate.toISOString().split('T')[0]; // YYYY-MM-DD format
        const { stdout } = await execAsync(`git log --since="${dateStr}" --name-status --pretty=format:"%H"`, { cwd: this.config.repositoryPath });
    ...
    async getCommitInfo(commitHash) {
        try {
            ...
            const { stdout } = await execAsync(`git show --name-status --pretty=format:"%H|%an|%ae|%ai|%s" ${hash}`, { cwd: this.config.repositoryPath });
            ...
            logger_js_1.logger.warn('Failed to get commit info', { commitHash, error });
            return null;
        }
    }
    /**
    ...
    async getRecentCommits(limit = 10) {
        try {
            const { stdout } = await execAsync(`git log --oneline -${limit} --pretty=format:"%H"`, { cwd: this.config.repositoryPath });
    ...
    async markCommitProcessed(commitHash) {
        ...
        logger_js_1.logger.debug('Marked commit as processed', { commitHash });
    }
    /**
    ...
    async loadLastProcessedCommit() {
        try {
            ...
            logger_js_1.logger.debug('No previous commit found or repository empty');
        }
    }
    /**
     * Get untracked files
    ...
    async getUntrackedFiles() {
        const { stdout } = await execAsync('git ls-files --others --exclude-standard', { cwd: this.config.repositoryPath });
        const changes = [];
        if (stdout.trim()) {
            ...
            for (const filePath of files) {
                if (this.shouldIncludeFile(filePath)) {
                    try {
                        ...
                        logger_js_1.logger.debug('Failed to stat untracked file', { filePath, error });
                    }
                }
            }
        }
     ...
     */
    async createFileChangeFromStatus(status, filePath) {
     ...
     */
    async createFileChangeFromGitStatus(status, filePath) {
        let changeType;
        switch (status) {
            case 'A':
                changeType = 'added';
     ...
     */
    async createFileChangeFromPath(filePath, changeType) {
        const fullPath = path.join(this.config.repositoryPath, filePath);
        try {
...
exports.GitChangeTracker = GitChangeTracker;
//# sourceMappingURL=git-change-tracker.js.map