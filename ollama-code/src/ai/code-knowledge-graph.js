...
exports.CodeKnowledgeGraph = void 0;
...
class CodeKnowledgeGraph {
    constructor(aiClient, projectContext, config = {}) {
        ...
        this.bestPracticeIndex = new Map();
        this.dataFlowIndex = new Map();
        this.lastOptimization = new Date();
        // Indexing and pattern recognition
        this.nodeTypePatterns = new Map();
        this.edgeTypePatterns = new Map();
    ...
    async initializePatternRecognition() {
        ...
        this.architecturalPatterns.set('service_layer', {
            ...
            validator: (nodes, edges) => {
                const serviceNodes = nodes.filter(n => n.name.toLowerCase().includes('service') ||
        ...
        this.architecturalPatterns.set('repository', {
            ...
            validator: (nodes, edges) => {
                const repoNodes = nodes.filter(n => n.name.toLowerCase().includes('repository') ||
        ...
        this.architecturalPatterns.set('crud_operations', {
            ...
            validator: (nodes, edges) => {
                ...
                const crudNodes = nodes.filter(n => crudMethods.some(method => n.name.toLowerCase().includes(method)));
    ...
    filterRelevantFiles(files) {
        ...
        return files.filter(fileInfo => {
            ...
            const hasExcludedDir = pathParts.some((part) => excludedDirectories.includes(part) || part.startsWith('.'));
            if (hasExcludedDir) {
                return false;
            }
     ...
     */
    async indexFileElements(filePath) {
        const elements = [];
        try {
    ...
    async buildFileRelationships() {
        ...
        for (const fileInfo of filteredFiles) {
            try {
                ...
                const content = await fs.promises.readFile(fullPath, 'utf8');
                const fileNode = Array.from(this.nodeIndex.values()).find(n => n.type === 'file' && n.properties.path === fileInfo.relativePath);
                if (!fileNode)
                    continue;
    ...
    async buildElementRelationships() {
        ...
        for (const node of this.nodeIndex.values()) {
            if (node.type !== 'file' && node.properties.file) {
                const fileNode = Array.from(this.nodeIndex.values()).find(n => n.type === 'file' && n.properties.path === node.properties.file);
                if (fileNode) {
                    edges.push({
            ...
            if (node.type === 'class' && node.properties.extends) {
                const parentClass = Array.from(this.nodeIndex.values()).find(n => n.type === 'class' && n.name === node.properties.extends);
                if (parentClass) {
                    edges.push({
            ...
            if (node.type === 'class' && node.properties.implements) {
                for (const interfaceName of node.properties.implements) {
                    const interfaceNode = Array.from(this.nodeIndex.values()).find(n => n.type === 'interface' && n.name === interfaceName);
                    if (interfaceNode) {
                        edges.push({
    ...
    async identifyPatterns() {
        ...
        try {
            for (const [patternType, definition] of this.architecturalPatterns) {
                const patternNodes = Array.from(this.nodeIndex.values()).filter(node => definition.nodeSignatures.includes(node.type));
                const patternEdges = Array.from(this.edgeIndex.values()).filter(edge => definition.edgeSignatures.includes(edge.type));
        ...
        try {
            ...
            for (const sourceNode of this.nodeIndex.values()) {
                if (sourceNode.type === 'function' || sourceNode.type === 'class') {
                    const outgoingEdges = Array.from(this.edgeIndex.values()).filter(edge => edge.source === sourceNode.id && (edge.type === 'calls' || edge.type === 'uses'));
    ...
    async queryGraph(query, options = {}) {
        ...
        try {
            ...
            let relevantNodes = Array.from(this.nodeIndex.values()).filter(node => {
                const nameMatch = queryWords.some(word => node.name.toLowerCase().includes(word));
            ...
            const relevantEdges = Array.from(this.edgeIndex.values()).filter(edge => nodeIds.has(edge.source) || nodeIds.has(edge.target));
            // Get related patterns
            const relevantPatterns = options.includePatterns ?
                Array.from(this.patternIndex.values()).filter(pattern => pattern.nodes.some(nodeId => nodeIds.has(nodeId))) : [];
            // Get related best practices
            const relevantBestPractices = options.includeBestPractices ?
                Array.from(this.bestPracticeIndex.values()).filter(practice => relevantPatterns.some(pattern => practice.applicablePatterns.includes(pattern.type))) : [];
            // Get related data flows
            const relevantDataFlows = Array.from(this.dataFlowIndex.values()).filter(flow => nodeIds.has(flow.source) || nodeIds.has(flow.target));
            const result = {
                nodes: relevantNodes,
                edges: relevantEdges,
    ...
    async findRelatedCode(elementId, options = {}) {
        ...
        try {
            ...
            const directEdges = Array.from(this.edgeIndex.values()).filter(edge => edge.source === elementId || edge.target === elementId);
            ...
            const queue = directRelations.map(rel => ({
                id: rel.id,
                distance: 1,
            ...
            while (queue.length > 0 && indirectRelations.length < 100) {
                ...
                if (current.distance >= maxDepth)
                    continue;
                const nextEdges = Array.from(this.edgeIndex.values()).filter(edge => edge.source === current.id || edge.target === current.id);
            ...
            const relatedPatterns = options.includePatterns ?
                Array.from(this.patternIndex.values()).filter(pattern => pattern.nodes.includes(elementId) ||
                    pattern.nodes.some(nodeId => directRelations.some(rel => rel.id === nodeId) ||
        ...
        try {
            ...
            while (queue.length > 0) {
                ...
                if (!visited.has(current.node)) {
                    ...
                    const outgoingEdges = Array.from(this.edgeIndex.values()).filter(edge => edge.source === current.node &&
     ...
     */
    generatePatternBasedSuggestions(pattern) {
        const suggestions = [];
        switch (pattern.type) {
            case 'service_layer':
                suggestions.push({
     ...
     */
    generateNodeBasedSuggestions(node) {
        const suggestions = [];
    ...
    generateArchitectureSuggestions() {
        ...
        const serviceClasses = Array.from(this.nodeIndex.values()).filter(node => node.type === 'class' && node.name.toLowerCase().includes('service'));
        if (serviceClasses.length > 3) {
            const serviceLayerPatterns = Array.from(this.patternIndex.values()).filter(p => p.type === 'service_layer');
    ...
    async removeNode(nodeId) {
        ...
        const edgesToRemove = Array.from(this.edgeIndex.entries()).filter(([_, edge]) => edge.source === nodeId || edge.target === nodeId);
    ...
    calculatePatternConfidence(nodes, edges, definition) {
    ...
    assessPatternComplexity(nodes, edges) {
    ...
    calculateQueryConfidence(nodes, query) {
        if (nodes.length === 0)
            return 0;
        const queryWords = query.toLowerCase().split(/\s+/);
    ...
    calculateRelationshipConfidence(directRels, indirectRels) {
        if (directRels.length === 0 && indirectRels.length === 0)
            return 0;
...
exports.CodeKnowledgeGraph = CodeKnowledgeGraph;
//# sourceMappingURL=code-knowledge-graph.js.map