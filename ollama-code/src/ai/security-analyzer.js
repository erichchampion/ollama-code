...
class SecurityAnalyzer {
    ...
    async analyzeProject(projectPath, options = {}) {
        ...
        const { includePatterns = ['**/*.js', '**/*.ts', '**/*.jsx', '**/*.tsx', '**/*.py', '**/*.java', '**/*.php', '**/*.rb'], excludePatterns = (0, file_patterns_js_1.getDefaultExcludePatterns)(), respectGitIgnore = true, checkDependencies = true, enableCustomRules = true, severityThreshold = 'info', maxFileSize = 1024 * 1024, // 1MB
        projectRoot = projectPath } = options;
        logger_js_1.logger.info(`Starting security analysis of project: ${projectPath}`);
        ...
        logger_js_1.logger.info(`Analyzing ${files.length} files for security vulnerabilities`);
        // Analyze each file
        let analyzedFiles = 0;
        ...
        if (checkDependencies) {
            try {
                ...
                logger_js_1.logger.warn('Failed to analyze dependencies:', error);
            }
        }
        const result = {
        ...
        logger_js_1.logger.info(`Security analysis completed: ${result.summary.vulnerabilities} vulnerabilities found in ${result.executionTime}ms`);
        return result;
    }
    /**
    ...
    loadDefaultRules() {
        ...
        logger_js_1.logger.info(`Loaded ${this.rules.length} security rules`);
    }
    /**
    ...
    async analyzeDependencies(projectPath) {
        ...
        try {
            ...
            for (const [pkg, version] of Object.entries(dependencies)) {
                ...
                if (this.isKnownVulnerablePackage(pkg, version)) {
            ...
            logger_js_1.logger.debug('No package.json found or unable to parse');
        }
        return vulnerabilities;
    }
    /**
     ...
     */
    createSummary(vulnerabilities, dependencyVulnerabilities, totalFiles) {
        const summary = {
            totalFiles,
     ...
     */
    getExploitabilityDescription(severity) {
        switch (severity) {
     ...
     */
    isKnownVulnerablePackage(pkg, version) {
    ...
    addCustomRule(rule) {
        ...
        logger_js_1.logger.info(`Added custom security rule: ${rule.id}`);
    }
    /**
