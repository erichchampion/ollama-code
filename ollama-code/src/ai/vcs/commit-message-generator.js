...
class CommitMessageGenerator {
    ...
    async generateCommitMessage() {
        try {
            logger_js_1.logger.info('Generating commit message from staged changes');
            // Analyze staged changes
            const analysis = await this.analyzeChanges();
            // Generate message based on style
            const message = await this.generateMessage(analysis);
            logger_js_1.logger.info('Commit message generated successfully', {
                type: message.type,
            ...
            logger_js_1.logger.error('Failed to generate commit message', error);
            throw error;
        }
    }
    /**
    ...
    async generateFromDiff(diffText) {
        try {
            logger_js_1.logger.info('Generating commit message from provided diff');
            const analysis = await this.analyzeDiff(diffText);
            const message = await this.generateMessage(analysis);
            return message;
        }
        catch (error) {
            logger_js_1.logger.error('Failed to generate commit message from diff', error);
            throw error;
        }
    }
    /**
    ...
    async analyzeChanges() {
        try {
            ...
            logger_js_1.logger.error('Failed to analyze staged changes', error);
            throw error;
        }
    }
    /**
    ...
    async analyzeDiff(diffText) {
        try {
            ...
            logger_js_1.logger.error('Failed to analyze diff', error);
            throw error;
        }
    }
    /**
     ...
     */
    determineOverallChangeType(files) {
        if (files.length === 0)
            return 'chore';
        const changeTypeCounts = new Map();
        files.forEach(file => {
    ...
    determineScope(files) {
        if (!this.config.includeScope || files.length === 0)
            return null;
        // Get all scopes
     ...
     */
    generateSuggestions(files, commitType) {
        const suggestions = [];
     ...
     */
    async generateMessage(analysis) {
        try {
            let message;
            let alternatives = [];
     ...
     */
    generateDescriptiveMessage(analysis) {
     ...
     */
    generateDescriptiveSubject(analysis) {
     ...
     */
    generateConventionalAlternatives(analysis) {
        const alternatives = [];
        const { overallChangeType: type, scope } = analysis;
     ...
     */
    generateDescriptiveAlternatives(analysis) {
        return [
     ...
     */
    generateMinimalAlternatives(analysis) {
        return [
            this.generateMinimalDescription(analysis),
            this.generateSubject(analysis),
     ...
     */
    generateMinimalDescription(analysis) {
     ...
     */
    async enhanceWithAI(message, analysis, alternatives) {
        try {
            if (!this.aiClient) {
...
`;
            ...
            logger_js_1.logger.debug('AI enhancement failed, using original message', error);
            return { message, alternatives };
        }
    }
    /**
    ...
    updateConfig(updates) {
        ...
        logger_js_1.logger.info('Commit message generator configuration updated', updates);
    }
    /**
