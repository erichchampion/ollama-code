...
class ProjectContext {
     ...
     */
    async initialize() {
        logger_js_1.logger.info(`Initializing project context for: ${this.projectRoot}`);
        try {
            await this.analyzeProjectStructure();
            await this.detectDependencies();
            this.setupFileWatchers();
            logger_js_1.logger.info('Project context initialized successfully');
        }
        catch (error) {
            logger_js_1.logger.error('Failed to initialize project context:', error);
            throw error;
        }
    }
    /**
    ...
    async analyzeProjectStructure() {
        const filesystem = index_js_1.toolRegistry.get('filesystem');
        if (!filesystem) {
        ...
        logger_js_1.logger.debug(`Analyzed ${this.structure.files.size} files in project structure`);
    }
    /**
    ...
    async detectDependencies() {
        ...
        const filesToAnalyze = Array.from(this.structure.files.keys())
            ...
            .slice(0, constants_js_1.MAX_FILES_FOR_ANALYSIS); // Limit analysis to avoid overwhelming
        for (const filePath of filesToAnalyze) {
            try {
                ...
                logger_js_1.logger.debug(`Failed to analyze dependencies for ${filePath}:`, error);
            }
        }
    }
    /**
    ...
    extractDependencies(content, language) {
        ...
        try {
            ...
            logger_js_1.logger.debug('Error extracting dependencies:', error);
        }
        return { imports, exports, dependencies };
    }
    /**
    ...
    setupFileWatchers() {
        try {
            ...
            ].slice(0, constants_js_1.MAX_FILE_WATCHERS); // Limit number of watchers
            for (const file of watchFiles) {
                try {
                    const fullPath = path.join(this.projectRoot, file);
                    const watcher = (0, fs_1.watch)(fullPath, (eventType) => {
                        if (eventType === 'change') {
                    ...
                    logger_js_1.logger.debug(`Failed to watch file ${file}:`, error);
                }
            }
        }
        catch (error) {
            logger_js_1.logger.error('Error setting up file watchers:', error);
            // Cleanup any partial watchers that were created
            this.cleanup();
            throw error;
        }
        logger_js_1.logger.debug(`Setup ${this.fileWatchers.size} file watchers`);
    }
    /**
    ...
    async getRelevantContext(query, maxTokens) {
        ...
        const recentConversation = this.conversationHistory.slice(-constants_js_1.MAX_CONVERSATION_HISTORY);
        // Calculate token usage (simplified)
        const totalTokens = this.estimateTokens(relevantFiles, recentConversation);
        return {
            files: relevantFiles.slice(0, constants_js_1.MAX_FILES_LIMIT), // Limit files
    ...
    generateStructureOverview() {
        const dirs = this.structure.directories.slice(0, constants_js_1.MAX_FILES_LIMIT);
        const files = this.structure.configFiles.concat(this.structure.entryPoints).slice(0, constants_js_1.MAX_FILES_LIMIT);
        return `
...
`.trim();
    ...
    cleanup() {
        ...
        logger_js_1.logger.debug('Project context cleanup completed');
    }
