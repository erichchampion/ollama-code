...
class HoverProvider {
     ...
     */
    analyzeElement(document, position) {
        ...
        const contextRange = new vscode.Range(Math.max(0, position.line - 10), 0, Math.min(document.lineCount - 1, position.line + 10), 0);
        ...
        const isTest = codeAnalysisUtils_1.CodeAnalysisUtils.isTestFunction(word);
        return {
            elementType,
            name: word,
            context,
     ...
     */
    determineElementType(word, lineText, context, languageId) {
        // Function patterns
        const funcPatterns = [
     ...
     */
    extractSignature(word, context, elementType) {
        if (elementType !== 'function' && elementType !== 'method') {
            return undefined;
        }
     ...
     */
    calculateElementComplexity(word, context) {
        ...
        return codeAnalysisUtils_1.CodeAnalysisUtils.calculateComplexity(functionCode);
    }
    /**
     * Check if element is exported
     */
    isExported(word, context) {
     ...
     */
    async generateHoverContent(document, analysis, token) {
        try {
            ...
            const result = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('Hover timeout')), 3000))
            ]);
     ...
     */
    createAnalysisPrompt(document, analysis) {
     ...
     */
    formatHoverContent(analysis, aiExplanation) {
        ...
        if (analysis.isExported)
            badges.push('`exported`');
        if (analysis.isAsync)
            badges.push('`async`');
        if (analysis.isTest)
            badges.push('`test`');
        if (analysis.complexity && analysis.complexity > 10)
            badges.push('`high complexity`');
        if (badges.length > 0) {
        ...
        if (analysis.complexity !== undefined) {
            ...
            if (analysis.complexity > analysisConstants_1.CODE_METRICS_THRESHOLDS.HIGH_COMPLEXITY) {
     ...
     */
    getElementRange(document, position, analysis) {
        ...
        if (analysis.elementType === 'function' || analysis.elementType === 'class' || analysis.elementType === 'method') {
            ...
            if (declarationStart !== -1) {
                return new vscode.Range(position.line, declarationStart, position.line, declarationStart + analysis.name.length);
            }
        }
     ...
     */
    createCacheKey(document, analysis) {
