...
class SecurityAnalyzer {
     ...
     */
    generateReviewReport(vulnerabilities) {
     ...
     */
    generateSummary(vulnerabilities, severityCounts) {
        if (vulnerabilities.length === 0) {
            return securityTestConstants_1.SUMMARY_NO_ISSUES;
        }
     ...
     */
    classifyBySeverity(vulnerabilities) {
     ...
     */
    classifyByCategory(vulnerabilities) {
        const categories = {};
        for (const vuln of vulnerabilities) {
    ...
    generateRecommendations(vulnerabilities) {
        ...
        for (const [key, vulns] of byCategory) {
            ...
            if (!seenRecommendations.has(recKey)) {
                ...
                recommendations.push({
                    ...
                    examples: vulns.slice(0, securityTestConstants_1.MAX_RECOMMENDATION_EXAMPLES).map(v => ({
                        file: v.file,
                        line: v.line,
                        code: v.code,
                    })),
        ...
        const getSeverityOrder = (severity) => {
            return securityTestConstants_1.SEVERITY_ORDER[severity] ?? 999; // Default for unknown severities
        };
     ...
     */
    generatePositiveFindings(vulnerabilities) {
        const findings = [];
        const categories = new Set(vulnerabilities.map(v => v.category));
        // Check what's NOT vulnerable
        const allCategories = Object.values(securityTestConstants_1.VULNERABILITY_CATEGORIES);
        for (const category of allCategories) {
            if (!categories.has(category)) {
                const message = securityTestConstants_1.POSITIVE_FINDING_MESSAGES[category];
                if (message) {
        ...
        if (findings.length === 0 && vulnerabilities.length > 0) {
            findings.push(securityTestConstants_1.FALLBACK_POSITIVE_FINDING);
        }
        return findings;
    }
    /**
     ...
     */
    getActionableFiles(vulnerabilities) {
        ...
        const getPriorityOrder = (priority) => {
            return securityTestConstants_1.PRIORITY_ORDER[priority] ?? 999;
        };
     ...
     */
    calculateConfidenceScore(vulnerabilities) {
        if (vulnerabilities.length === 0) {
            return securityTestConstants_1.PERFECT_CONFIDENCE_SCORE;
        }
        const getSeverityWeight = (severity) => {
            if (severity === 'critical')
                return securityTestConstants_1.SEVERITY_WEIGHTS.CRITICAL;
            if (severity === 'high')
                return securityTestConstants_1.SEVERITY_WEIGHTS.HIGH;
            if (severity === 'medium')
                return securityTestConstants_1.SEVERITY_WEIGHTS.MEDIUM;
            if (severity === 'low')
                return securityTestConstants_1.SEVERITY_WEIGHTS.LOW;
            return securityTestConstants_1.SEVERITY_WEIGHTS.INFO;
        };
        let totalConfidence = 0;
        let totalWeight = 0;
        for (const vuln of vulnerabilities) {
            const confidenceLevel = vuln.confidence ?? 'medium';
            const confidence = securityTestConstants_1.CONFIDENCE_SCORE_VALUES[confidenceLevel.toUpperCase()] ?? securityTestConstants_1.CONFIDENCE_SCORE_VALUES.MEDIUM;
            const weight = getSeverityWeight(vuln.severity);
            totalConfidence += confidence * weight;
            totalWeight += weight;
        }
        return totalWeight > 0 ? Math.round(totalConfidence / totalWeight) : securityTestConstants_1.DEFAULT_CONFIDENCE_SCORE;
    }
    /**
     * Calculate quality metrics for a file
     */
    async calculateQualityMetrics(filePath) {
        try {
            const content = await fs_1.promises.readFile(filePath, 'utf8');
            ...
            const overallScore = Math.round(scores.complexity * weights.complexity +
     ...
     */
    async calculateMaintainabilityMetrics(content, complexity) {
        ...
        const maintainabilityIndex = Math.round(100 - (complexity.cyclomaticComplexity * 2) - (Math.log(loc + 1) * 5) + (commentRatio * 20));
        // Count code smells
     ...
     */
    async calculateBestPracticeMetrics(content, filePath) {
     ...
     */
    async calculateTestCoverageMetrics(filePath) {
     ...
     */
    async calculateDocumentationMetrics(content) {
     ...
     */
    async calculateTypeSafetyMetrics(content, filePath) {
