...
suite('CodeLens Provider Tests', () => {
    ...
    setup(async function () {
        ...
        mockClient = (0, providerTestHelper_1.createMockOllamaClient)();
        mockLogger = (0, providerTestHelper_1.createMockLogger)();
        // Create provider
        codeLensProvider = new codeLensProvider_1.CodeLensProvider(mockClient, mockLogger);
        // Create test workspace
        testWorkspacePath = await (0, extensionTestHelper_1.createTestWorkspace)('codelens-tests');
...
function complexFunction(a, b, c) {
  ...
  return a + b + c;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'complex.js', complexFunction);
            ...
            const complexityLens = lenses.find(lens => lens.command?.title.includes('⚠️ Complexity:') &&
                lens.command?.title.includes('Consider refactoring'));
            assert.ok(complexityLens, 'Should have high complexity warning lens');
            assert.strictEqual(complexityLens.command?.command, 'ollama-code.refactor', 'Should trigger refactor command');
        });
...
function moderateFunction(x) {
  ...
  return x;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'moderate.js', moderateFunction);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const lenses = await codeLensProvider.provideCodeLenses(document, cancellationToken);
            const complexityLens = lenses.find(lens => lens.command?.title.includes('📊 Complexity:') &&
                !lens.command?.title.includes('Consider refactoring'));
            assert.ok(complexityLens, 'Should have medium complexity info lens');
            assert.strictEqual(complexityLens.command?.command, 'ollama-code.analyze', 'Should trigger analyze command');
        });
...
function simpleFunction(x) {
  return x * 2;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'simple.js', simpleFunction);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const lenses = await codeLensProvider.provideCodeLenses(document, cancellationToken);
            const complexityLens = lenses.find(lens => lens.command?.title.includes('Complexity:'));
    ...
    suite('Function Size CodeLens', () => {
        test('Should show line count warning for long functions', async function () {
            ...
            const lines = Array(providerTestHelper_1.TEST_DATA_CONSTANTS.LONG_FUNCTION_LINE_COUNT).fill('  console.log("line");').join('\n');
            const longFunction = `
function longFunction() {
${lines}
}
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'long.js', longFunction);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const lenses = await codeLensProvider.provideCodeLenses(document, cancellationToken);
            const lineLens = lenses.find(lens => lens.command?.title.includes('📏 Lines:') &&
                lens.command?.title.includes('Consider breaking down'));
            assert.ok(lineLens, 'Should have line count warning lens');
            assert.strictEqual(lineLens.command?.command, 'ollama-code.refactor', 'Should trigger refactor command');
        });
...
function manyParams(a, b, c, d, e, f, g) {
  return a + b + c + d + e + f + g;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'params.js', manyParamsFunction);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const lenses = await codeLensProvider.provideCodeLenses(document, cancellationToken);
            const paramLens = lenses.find(lens => lens.command?.title.includes('📝 Parameters:') &&
                lens.command?.title.includes('Too many parameters'));
            assert.ok(paramLens, 'Should have parameter count warning lens');
            assert.strictEqual(paramLens.command?.command, 'ollama-code.refactor', 'Should trigger refactor command');
        });
    });
...
function third() {
  return 3;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'multiple.js', multipleFunctions);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const lenses = await codeLensProvider.provideCodeLenses(document, cancellationToken);
            const aiLenses = lenses.filter(lens => lens.command?.title.includes('🤖 Get AI insights'));
            ...
            aiLenses.forEach(lens => {
                assert.strictEqual(lens.command?.command, 'ollama-code.analyzeFunction', 'Should trigger analyzeFunction command');
            });
        });
    });
...
function complex2(y) {
  ...
  return y;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'complex-file.js', complexFile);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const lenses = await codeLensProvider.provideCodeLenses(document, cancellationToken);
            const fileLens = lenses.find(lens => lens.command?.title.includes('📊 File Complexity:') &&
                lens.command?.title.includes('Consider refactoring'));
            assert.ok(fileLens, 'Should have file complexity warning lens');
            assert.strictEqual(fileLens.command?.command, 'ollama-code.analyzeFile', 'Should trigger analyzeFile command');
...
function add(a, b) {
  return a + b;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'testable.js', fileWithFunctions);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const lenses = await codeLensProvider.provideCodeLenses(document, cancellationToken);
            const testLens = lenses.find(lens => lens.command?.title.includes('🧪 Generate tests'));
            assert.ok(testLens, 'Should have test generation lens');
            assert.strictEqual(testLens.command?.command, 'ollama-code.generateTests', 'Should trigger generateTests command');
...
function processData(input) {
  return input.toLowerCase();
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'secure.js', fileWithFunctions);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const lenses = await codeLensProvider.provideCodeLenses(document, cancellationToken);
            const securityLens = lenses.find(lens => lens.command?.title.includes('🔒 Run security analysis'));
            assert.ok(securityLens, 'Should have security analysis lens');
            assert.strictEqual(securityLens.command?.command, 'ollama-code.securityAnalysis', 'Should trigger securityAnalysis command');
...
function greet(name: string): string {
  return \`Hello, \${name}!\`;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'test.ts', tsFunction);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const lenses = await codeLensProvider.provideCodeLenses(document, cancellationToken);
            assert.ok(lenses.length > 0, 'Should provide lenses for TypeScript');
            const aiLens = lenses.find(lens => lens.command?.title.includes('🤖 Get AI insights'));
...
def calculate(x, y):
    ...
    return 0
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'test.py', pyFunction);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const lenses = await codeLensProvider.provideCodeLenses(document, cancellationToken);
...
It should not trigger any code lenses.
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'test.txt', textFile);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const lenses = await codeLensProvider.provideCodeLenses(document, cancellationToken);
    ...
    suite('Connection and Error Handling', () => {
        test('Should return empty array when client is disconnected', async function () {
            ...
            const provider = new codeLensProvider_1.CodeLensProvider(disconnectedClient, mockLogger);
            const simpleFunction = `
function test() {
  return 42;
}
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'disconnected.js', simpleFunction);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
        ...
        test('Should handle cancellation token gracefully', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const manyFunctions = Array(providerTestHelper_1.TEST_DATA_CONSTANTS.STRESS_TEST_FUNCTION_COUNT).fill(0).map((_, i) => `
function func${i}() {
  return ${i};
}
      `).join('\n');
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'cancel.js', manyFunctions);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
    ...
    suite('CodeLens Refresh', () => {
        test('Should fire refresh event when refresh() is called', async function () {
            ...
            await (0, extensionTestHelper_1.sleep)(test_constants_1.EXTENSION_TEST_CONSTANTS.POLLING_INTERVAL);
