...
suite('DocumentSymbol Provider Tests', () => {
    ...
    setup(async function () {
        ...
        // Create provider
        symbolProvider = new documentSymbolProvider_1.DocumentSymbolProvider(mockClient, mockLogger);
        // Create test workspace
        testWorkspacePath = await (0, extensionTestHelper_1.createTestWorkspace)('document-symbol-tests');
...
class InternalHelper {
  static format(data: any): string {
    return JSON.stringify(data);
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'service.ts', tsCode);
            ...
            assert.ok(classes.some(c => c.name === 'UserService'), 'Should extract UserService class');
            assert.ok(classes.some(c => c.name === 'InternalHelper'), 'Should extract InternalHelper class');
        });
...
const processData = (data) => {
  return data.map(item => item.value);
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'utils.js', jsCode);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const symbols = await symbolProvider.provideDocumentSymbols(document, cancellationToken);
            ...
            assert.ok(functions.some(f => f.name === 'calculateTotal'), 'Should extract calculateTotal function');
            assert.ok(functions.some(f => f.name === 'fetchData'), 'Should extract fetchData function');
        });
...
export enum UserRole {
  ...
  Guest = 'guest'
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'types.ts', tsCode);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const symbols = await symbolProvider.provideDocumentSymbols(document, cancellationToken);
...
var debugMode = false;
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'config.ts', tsCode);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const symbols = await symbolProvider.provideDocumentSymbols(document, cancellationToken);
            ...
            assert.ok(constants.some(c => c.name === 'API_URL'), 'Should extract API_URL constant');
            assert.ok(variables.length >= 1, 'Should extract variables');
            assert.ok(variables.some(v => v.name === 'currentUser'), 'Should extract currentUser variable');
        });
    });
...
def test_processor():
    ...
    assert processor is not None
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'processor.py', pyCode);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const symbols = await symbolProvider.provideDocumentSymbols(document, cancellationToken);
            const classes = symbols.filter(s => s.kind === vscode.SymbolKind.Class);
            const functions = symbols.filter(s => s.kind === vscode.SymbolKind.Function);
            ...
            assert.ok(functions.some(f => f.name === 'fetch_remote_data'), 'Should extract fetch_remote_data function');
            assert.ok(functions.some(f => f.name === 'test_processor'), 'Should extract test_processor function');
        });
...
public class UserController {
    ...
    public User getUser(String id) {
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'UserController.java', javaCode);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const symbols = await symbolProvider.provideDocumentSymbols(document, cancellationToken);
            const classes = symbols.filter(s => s.kind === vscode.SymbolKind.Class);
            ...
            assert.ok(methods.some(m => m.name === 'getUser'), 'Should extract getUser method');
        });
...
func HandleRequest(req *Request) *Response {
    return &Response{}
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'handler.go', goCode);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const symbols = await symbolProvider.provideDocumentSymbols(document, cancellationToken);
            const functions = symbols.filter(s => s.kind === vscode.SymbolKind.Function);
            assert.ok(functions.length >= 2, 'Should extract Go functions');
            assert.ok(functions.some(f => f.name === 'ProcessData'), 'Should extract ProcessData function');
            assert.ok(functions.some(f => f.name === 'HandleRequest'), 'Should extract HandleRequest function');
        });
    });
...
export async function fetchUser(id: string) {
  ...
  return response.json();
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'async.ts', tsCode);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const symbols = await symbolProvider.provideDocumentSymbols(document, cancellationToken);
            const asyncFunc = symbols.find(s => s.name === 'fetchUser');
            assert.ok(asyncFunc, 'Should extract async function');
            assert.ok(asyncFunc.detail?.includes('async'), 'Should mark function as async in detail');
        });
...
function test_validation() {
  validate({});
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'tests.ts', tsCode);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const symbols = await symbolProvider.provideDocumentSymbols(document, cancellationToken);
            const testFuncs = symbols.filter(s => s.name.includes('test') && s.detail?.includes('test'));
...
function internalUtil() {}
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'exports.ts', tsCode);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const symbols = await symbolProvider.provideDocumentSymbols(document, cancellationToken);
            ...
            assert.ok(exportedSymbols.some(s => s.name === 'PublicService'), 'Should mark PublicService as exported');
            assert.ok(exportedSymbols.some(s => s.name === 'publicApi'), 'Should mark publicApi as exported');
        });
...
function complex(x) {
  ...
  return x;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'complexity.js', complexFunc);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const symbols = await symbolProvider.provideDocumentSymbols(document, cancellationToken);
    ...
    suite('Error Handling and Edge Cases', () => {
        test('Should handle empty files gracefully', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const emptyFile = '';
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'empty.ts', emptyFile);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const symbols = await symbolProvider.provideDocumentSymbols(document, cancellationToken);
...
function func${i}() {
  return ${i};
      ...
      `).join('\n');
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'large.js', largeFile);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const tokenSource = new vscode.CancellationTokenSource();
            // Start parsing
        ...
        test('Should return empty array when disconnected from client', async function () {
            ...
            const provider = new documentSymbolProvider_1.DocumentSymbolProvider(disconnectedClient, mockLogger);
            const tsCode = `
function test() {
  return 42;
}
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'disconnected.ts', tsCode);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
...
//# sourceMappingURL=documentSymbol.provider.test.js.map