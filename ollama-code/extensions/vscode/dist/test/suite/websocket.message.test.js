...
suite('WebSocket Message Processing Tests', () => {
    const TEST_PORT = test_constants_1.WEBSOCKET_TEST_CONSTANTS.PORTS.MESSAGE_TESTS;
    ...
    suite('JSON Message Parsing', () => {
        test('Should parse valid JSON messages correctly', async function () {
            ...
            await (0, test_utils_1.sleep)(test_constants_1.TEST_DELAYS.MESSAGE_PROCESSING);
            const received = mockServer.getReceivedMessages();
            assert.strictEqual(received.length, 1, 'Should receive 1 message');
            assert.deepStrictEqual(received[0], validMessage, 'Parsed message should match original');
        });
        test('Should handle messages with nested objects', async function () {
            ...
            await (0, test_utils_1.sleep)(test_constants_1.TEST_DELAYS.MESSAGE_PROCESSING);
            const received = mockServer.getReceivedMessages();
            assert.deepStrictEqual(received[0], nestedMessage, 'Nested objects should be preserved');
        });
        test('Should handle messages with arrays', async function () {
            ...
            assert.deepStrictEqual(received[0].tags, ['test', 'batch', 'operation'], 'String arrays should be preserved');
        });
    });
    suite('Invalid Message Handling', () => {
        test('Should handle malformed JSON gracefully', async function () {
            ...
            await (0, test_utils_1.sleep)(test_constants_1.TEST_DELAYS.MESSAGE_PROCESSING);
            const received = mockServer.getReceivedMessages();
            assert.strictEqual(received.length, 1, 'Server should receive malformed message');
            assert.strictEqual(received[0], malformedJSON, 'Server should receive message as string');
        });
        ...
        test('Should handle special characters in messages', async function () {
            ...
            await (0, test_utils_1.sleep)(test_constants_1.TEST_DELAYS.MESSAGE_PROCESSING);
            const received = mockServer.getReceivedMessages();
            assert.strictEqual(received[0].content, messageWithSpecialChars.content, 'Special characters should be preserved');
            assert.strictEqual(received[0].unicode, messageWithSpecialChars.unicode, 'Unicode should be preserved');
            assert.strictEqual(received[0].emoji, messageWithSpecialChars.emoji, 'Emoji should be preserved');
        });
    });
    suite('Message Routing and Correlation', () => {
        test('Should handle request-response correlation with request IDs', async function () {
            ...
            requests.forEach((original, index) => {
                assert.strictEqual(received[index].id, original.id, `Request ${index + 1} should maintain ID`);
            });
        });
        test('Should handle concurrent message processing', async function () {
            ...
            const received = mockServer.getReceivedMessages();
            assert.strictEqual(received.length, MESSAGE_COUNT, `Should receive all ${MESSAGE_COUNT} messages`);
            // Verify all sequence numbers are present
            const sequences = received.map((msg) => msg.sequence).sort((a, b) => a - b);
            const expectedSequences = Array.from({ length: MESSAGE_COUNT }, (_, i) => i);
            assert.deepStrictEqual(sequences, expectedSequences, 'All sequence numbers should be present');
        });
        test('Should handle message type routing', async function () {
            ...
            assert.deepStrictEqual(types, ['command', 'query', 'event', 'notification'], 'All message types should be preserved');
        });
    });
    suite('Message Size and Performance', () => {
        test('Should handle large messages (>1MB)', async function () {
            ...
            assert.strictEqual(received[0].payload.length, largeData.length, 'Large payload should be intact');
        });
        test('Should handle message queue with rapid messages', async function () {
            ...
            const received = mockServer.getReceivedMessages();
            assert.strictEqual(received.length, RAPID_MESSAGE_COUNT, `Should receive all ${RAPID_MESSAGE_COUNT} rapid messages`);
            // Verify order is preserved
            for (let i = 0; i < RAPID_MESSAGE_COUNT; i++) {
                assert.strictEqual(received[i].index, i, `Message ${i} should have correct index`);
            }
        });
        test('Should handle empty payload messages efficiently', async function () {
            ...
            await (0, test_utils_1.sleep)(test_constants_1.TEST_DELAYS.MESSAGE_PROCESSING);
            const received = mockServer.getReceivedMessages();
            assert.strictEqual(received.length, 4, 'Should receive all empty payload messages');
        });
    });
    suite('Message Validation and Error Handling', () => {
        test('Should handle unknown message types gracefully', async function () {
            ...
            await (0, test_utils_1.sleep)(test_constants_1.TEST_DELAYS.MESSAGE_PROCESSING);
            const received = mockServer.getReceivedMessages();
            assert.strictEqual(received.length, 1, 'Should receive unknown message type');
            assert.strictEqual(received[0].type, 'completely-unknown-type-xyz', 'Unknown type should be preserved');
        });
        ...
        test('Should handle messages with extra unexpected fields', async function () {
            ...
            await (0, test_utils_1.sleep)(test_constants_1.TEST_DELAYS.MESSAGE_PROCESSING);
            const received = mockServer.getReceivedMessages();
            assert.ok(received[0].unexpectedField1, 'Extra fields should be preserved');
            assert.deepStrictEqual(received[0], messageWithExtras, 'All fields should be preserved');
        });
    });
    suite('Binary and Complex Data', () => {
        test('Should handle messages with binary-like data (base64)', async function () {
            ...
            await (0, test_utils_1.sleep)(test_constants_1.TEST_DELAYS.MESSAGE_PROCESSING);
            const received = mockServer.getReceivedMessages();
            assert.strictEqual(received[0].data, binaryData, 'Base64 data should be preserved');
            assert.strictEqual(Buffer.from(received[0].data, 'base64').toString(), 'Hello World', 'Base64 data should be decodable');
        });
        test('Should handle messages with numeric precision', async function () {
            ...
            assert.strictEqual(received[0].float.toFixed(11), '3.14159265359', 'Float precision should be maintained');
            assert.strictEqual(received[0].large, 9007199254740991, 'Large numbers should be exact');
        });
    });
});
//# sourceMappingURL=websocket.message.test.js.map