...
suite('Chat Panel Integration Tests', () => {
    ...
    setup(async function () {
        ...
        mockClient = (0, providerTestHelper_1.createMockOllamaClient)(true, (0, providerTestHelper_1.createChatAIHandler)());
        mockLogger = (0, providerTestHelper_1.createMockLogger)();
        // Create chat panel
        chatPanel = new MockChatPanel(mockClient, mockLogger);
        // Create test workspace
        testWorkspacePath = await (0, extensionTestHelper_1.createTestWorkspace)('chat-panel-tests');
    });
    ...
    suite('Chat Interface', () => {
        ...
        test('Should send and receive messages', async function () {
            ...
            assert.strictEqual(messages.length, providerTestHelper_1.TEST_DATA_CONSTANTS.CHAT_TEST_COUNTS.SINGLE_EXCHANGE, 'Should have user message and AI response');
        ...
        test('Should maintain conversation history', async function () {
            ...
            const messages = chatPanel.getMessages();
            assert.strictEqual(messages.length, providerTestHelper_1.TEST_DATA_CONSTANTS.CHAT_TEST_COUNTS.TRIPLE_EXCHANGE, 'Should have 3 user messages + 3 assistant responses');
            const userMessages = chatPanel.findMessagesByRole('user');
            const assistantMessages = chatPanel.findMessagesByRole('assistant');
            assert.strictEqual(userMessages.length, providerTestHelper_1.TEST_DATA_CONSTANTS.CHAT_TEST_COUNTS.TRIPLE_USER_MESSAGES, 'Should have 3 user messages');
            assert.strictEqual(assistantMessages.length, providerTestHelper_1.TEST_DATA_CONSTANTS.CHAT_TEST_COUNTS.TRIPLE_ASSISTANT_MESSAGES, 'Should have 3 assistant messages');
        });
        ...
        test('Should persist chat session', async function () {
            ...
            assert.strictEqual(messagesAfter, providerTestHelper_1.TEST_DATA_CONSTANTS.CHAT_TEST_COUNTS.DOUBLE_EXCHANGE, 'Should have 2 user messages + 2 assistant responses');
        });
        ...
        test('Should display conversation in chronological order', async function () {
            ...
            await (0, extensionTestHelper_1.sleep)(providerTestHelper_1.TEST_DATA_CONSTANTS.DELAYS.TIMESTAMP_DIFFERENTIATION);
            ...
            for (let i = 1; i < messages.length; i++) {
                assert.ok(messages[i].timestamp >= messages[i - 1].timestamp, 'Messages should be in chronological order');
            }
        });
        test('Should handle rapid message sending', async function () {
            ...
            const messages = chatPanel.getMessages();
            assert.strictEqual(messages.length, providerTestHelper_1.TEST_DATA_CONSTANTS.CHAT_TEST_COUNTS.TRIPLE_EXCHANGE, 'Should handle all rapid messages (3 user + 3 assistant)');
        });
        test('Should clear conversation history', async function () {
            ...
            await chatPanel.sendMessage('Message 1');
            await chatPanel.sendMessage('Message 2');
            assert.strictEqual(chatPanel.getMessageCount(), providerTestHelper_1.TEST_DATA_CONSTANTS.CHAT_TEST_COUNTS.DOUBLE_EXCHANGE, 'Should have messages before clear');
    ...
    suite('Chat Commands', () => {
        ...
        test('Should execute /clear command', async function () {
            ...
            await chatPanel.sendMessage('Message 1');
            await chatPanel.sendMessage('Message 2');
            assert.strictEqual(chatPanel.getMessageCount(), providerTestHelper_1.TEST_DATA_CONSTANTS.CHAT_TEST_COUNTS.DOUBLE_EXCHANGE, 'Should have 4 messages before clear');
            await chatPanel.sendMessage('/clear');
            // After /clear command, only the /clear message and its response remain
            assert.strictEqual(chatPanel.getMessageCount(), providerTestHelper_1.TEST_DATA_CONSTANTS.CHAT_TEST_COUNTS.SINGLE_EXCHANGE, 'Should only have /clear command and response');
            const lastMessage = chatPanel.getLastMessage();
            assert.ok(lastMessage.content.includes('Cleared'), 'Should confirm clearing');
            assert.ok(lastMessage.content.includes(String(providerTestHelper_1.TEST_DATA_CONSTANTS.CHAT_TEST_COUNTS.DOUBLE_EXCHANGE)), 'Should mention number of cleared messages');
        });
...
//# sourceMappingURL=chatPanel.integration.test.js.map