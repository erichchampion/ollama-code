...
suite('Diagnostic Provider Tests', () => {
    ...
    setup(async function () {
        ...
        mockClient = (0, providerTestHelper_1.createMockOllamaClient)(true, (0, providerTestHelper_1.createDiagnosticAIHandler)());
        mockLogger = (0, providerTestHelper_1.createMockLogger)();
        // Create provider
        diagnosticProvider = new diagnosticProvider_1.DiagnosticProvider(mockClient, mockLogger);
        // Create test workspace
        testWorkspacePath = await (0, extensionTestHelper_1.createTestWorkspace)('diagnostic-provider-tests');
    });
...
function processInput(userInput) {
  ...
  return result;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'eval.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            await diagnosticProvider.analyzeDiagnostics(document);
            await (0, extensionTestHelper_1.sleep)(100);
            const diagnostics = diagnosticProvider.getDiagnostics(document.uri);
            const evalDiagnostic = diagnostics.find(d => d.message.toLowerCase().includes('eval'));
            assert.ok(evalDiagnostic, 'Should detect eval() usage');
            assert.strictEqual(evalDiagnostic.severity, vscode.DiagnosticSeverity.Warning, 'Should be a warning');
            assert.strictEqual(evalDiagnostic.source, 'ollama-code-static', 'Should be from static analysis');
        });
...
function updateContent(html) {
  document.getElementById('output').innerHTML = html;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'innerHTML.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            await diagnosticProvider.analyzeDiagnostics(document);
            await (0, extensionTestHelper_1.sleep)(100);
            const diagnostics = diagnosticProvider.getDiagnostics(document.uri);
            const innerHTMLDiagnostic = diagnostics.find(d => d.message.toLowerCase().includes('innerhtml'));
            assert.ok(innerHTMLDiagnostic, 'Should detect innerHTML assignment');
            assert.strictEqual(innerHTMLDiagnostic.severity, vscode.DiagnosticSeverity.Warning, 'Should be a warning');
        });
...
function render(content) {
  document.write(content);
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'docwrite.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            await diagnosticProvider.analyzeDiagnostics(document);
            await (0, extensionTestHelper_1.sleep)(100);
            const diagnostics = diagnosticProvider.getDiagnostics(document.uri);
            const writeDignostic = diagnostics.find(d => d.message.toLowerCase().includes('document.write'));
...
function debugInfo(data) {
  ...
  return data;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'console.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            await diagnosticProvider.analyzeDiagnostics(document);
            await (0, extensionTestHelper_1.sleep)(100);
            const diagnostics = diagnosticProvider.getDiagnostics(document.uri);
            const consoleDiagnostic = diagnostics.find(d => d.message.toLowerCase().includes('console'));
            assert.ok(consoleDiagnostic, 'Should detect console.log statement');
            assert.strictEqual(consoleDiagnostic.severity, vscode.DiagnosticSeverity.Information, 'Should be informational');
        });
...
function processArray(items) {
  for (var i = 0; i < items.length; i++) {
    process(items[i]);
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'loop.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            await diagnosticProvider.analyzeDiagnostics(document);
            await (0, extensionTestHelper_1.sleep)(100);
            const diagnostics = diagnosticProvider.getDiagnostics(document.uri);
            const loopDiagnostic = diagnostics.find(d => d.message.toLowerCase().includes('array length') ||
...
function oldStyle() {
  ...
  return counter;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'var.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            await diagnosticProvider.analyzeDiagnostics(document);
            await (0, extensionTestHelper_1.sleep)(100);
            const diagnostics = diagnosticProvider.getDiagnostics(document.uri);
            const varDiagnostics = diagnostics.filter(d => d.message.toLowerCase().includes('var') &&
            ...
            assert.strictEqual(varDiagnostics[0].severity, vscode.DiagnosticSeverity.Hint, 'Should be a hint');
        });
...
function compare(a, b) {
  ...
  return false;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'equality.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            await diagnosticProvider.analyzeDiagnostics(document);
            await (0, extensionTestHelper_1.sleep)(100);
            const diagnostics = diagnosticProvider.getDiagnostics(document.uri);
            const equalityDiagnostic = diagnostics.find(d => d.message.includes('===') || d.message.toLowerCase().includes('strict equality'));
...
function alwaysTrue() {
  if (true) {
    return 'always executed';
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'true.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            await diagnosticProvider.analyzeDiagnostics(document);
            await (0, extensionTestHelper_1.sleep)(100);
            const diagnostics = diagnosticProvider.getDiagnostics(document.uri);
            const trueDiagnostic = diagnostics.find(d => d.message.toLowerCase().includes('always true'));
            assert.ok(trueDiagnostic, 'Should detect always-true condition');
            assert.strictEqual(trueDiagnostic.severity, vscode.DiagnosticSeverity.Warning, 'Should be a warning');
        });
...
function deadCode() {
  if (false) {
    return 'never executed';
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'false.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            await diagnosticProvider.analyzeDiagnostics(document);
            await (0, extensionTestHelper_1.sleep)(100);
            const diagnostics = diagnosticProvider.getDiagnostics(document.uri);
            const falseDiagnostic = diagnostics.find(d => d.message.toLowerCase().includes('always false') ||
                d.message.toLowerCase().includes('dead code'));
            assert.ok(falseDiagnostic, 'Should detect dead code');
            assert.strictEqual(falseDiagnostic.severity, vscode.DiagnosticSeverity.Error, 'Should be an error');
        });
    });
...
function complex(x) {
  ...
  return x;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'complex.ts', complexCode);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            await diagnosticProvider.analyzeDiagnostics(document);
            await (0, extensionTestHelper_1.sleep)(100);
            const diagnostics = diagnosticProvider.getDiagnostics(document.uri);
            const complexityDiagnostic = diagnostics.find(d => d.message.toLowerCase().includes('complexity') &&
                d.source === 'ollama-code-complexity');
            assert.ok(complexityDiagnostic, 'Should detect high complexity');
            assert.strictEqual(complexityDiagnostic.severity, vscode.DiagnosticSeverity.Warning, 'Should be a warning');
            assert.strictEqual(complexityDiagnostic.code, 'high-complexity', 'Should have high-complexity code');
        });
        test('Should detect long functions', async function () {
            ...
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'long.ts', longCode);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            await diagnosticProvider.analyzeDiagnostics(document);
            await (0, extensionTestHelper_1.sleep)(100);
            const diagnostics = diagnosticProvider.getDiagnostics(document.uri);
            const longDiagnostic = diagnostics.find(d => d.message.toLowerCase().includes('too long') &&
                d.code === 'long-function');
            assert.ok(longDiagnostic, 'Should detect long function');
            assert.strictEqual(longDiagnostic.severity, vscode.DiagnosticSeverity.Hint, 'Should be a hint');
        });
...
function manyParams(a, b, c, d, e, f, g) {
  return a + b + c + d + e + f + g;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'params.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            await diagnosticProvider.analyzeDiagnostics(document);
            await (0, extensionTestHelper_1.sleep)(100);
            const diagnostics = diagnosticProvider.getDiagnostics(document.uri);
            const paramsDiagnostic = diagnostics.find(d => d.message.toLowerCase().includes('too many parameters') &&
                d.code === 'too-many-params');
            assert.ok(paramsDiagnostic, 'Should detect too many parameters');
            assert.strictEqual(paramsDiagnostic.severity, vscode.DiagnosticSeverity.Hint, 'Should be a hint');
        });
    });
    suite('Diagnostic Management', () => {
        test('Should cache diagnostic results', async function () {
            ...
            assert.strictEqual(diagnostics1.length, diagnostics2.length, 'Cached diagnostics should match');
        });
        test('Should clear diagnostics for a specific document', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const code = 'function test() { var x = 5; }';
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'clear.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            await diagnosticProvider.analyzeDiagnostics(document);
            await (0, extensionTestHelper_1.sleep)(100);
        ...
        test('Should clear all diagnostics', async function () {
            ...
            const filePath1 = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'clear1.ts', code1);
            const filePath2 = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'clear2.ts', code2);
            const doc1 = await (0, extensionTestHelper_1.openDocument)(filePath1);
            const doc2 = await (0, extensionTestHelper_1.openDocument)(filePath2);
    ...
    suite('Language and File Type Filtering', () => {
        test('Should only apply language-specific rules to matching files', async function () {
            ...
            const tsPath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'ts.ts', code);
            const tsDoc = await (0, extensionTestHelper_1.openDocument)(tsPath);
            await diagnosticProvider.analyzeDiagnostics(tsDoc);
            await (0, extensionTestHelper_1.sleep)(100);
            const tsDiagnostics = diagnosticProvider.getDiagnostics(tsDoc.uri);
            const hasVarWarning = tsDiagnostics.some(d => d.message.toLowerCase().includes('var'));
        ...
        test('Should skip analysis for minified files', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const code = 'function test(){var x=5;}';
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'bundle.min.js', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            await diagnosticProvider.analyzeDiagnostics(document);
            await (0, extensionTestHelper_1.sleep)(100);
            const diagnostics = diagnosticProvider.getDiagnostics(document.uri);
    ...
    suite('Error Handling', () => {
        test('Should handle disconnected client gracefully', async function () {
            ...
            const provider = new diagnosticProvider_1.DiagnosticProvider(disconnectedClient, mockLogger);
            const code = 'function test() { var x = 5; }';
        ...
        test('Should handle AI analysis errors gracefully', async function () {
            ...
            const provider = new diagnosticProvider_1.DiagnosticProvider(errorClient, mockLogger);
...
//# sourceMappingURL=diagnostic.provider.test.js.map