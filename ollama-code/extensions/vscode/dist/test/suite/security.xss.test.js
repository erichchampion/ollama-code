...
suite('Security - XSS Vulnerabilities Tests', () => {
    ...
    setup(async function () {
        ...
        testWorkspacePath = await (0, extensionTestHelper_1.createTestWorkspace)('security-xss-tests');
    });
...
`;
            const vulnerabilities = await (0, securityTestHelper_1.testXSSDetection)(testWorkspacePath, 'xss-reflected-innerHTML.js', vulnerableCode);
            // Validate security metadata
            (0, securityTestHelper_1.assertSecurityMetadata)(vulnerabilities[0]);
            // Verify OWASP and CWE mappings
            assert.strictEqual(vulnerabilities[0].cweId, securityTestConstants_1.CWE_IDS.XSS);
            assert.ok(vulnerabilities[0].owaspCategory?.includes('A03:2021'));
            assert.strictEqual(vulnerabilities[0].category, securityTestConstants_1.VULNERABILITY_CATEGORIES.XSS);
...
`;
            const vulnerabilities = await (0, securityTestHelper_1.testXSSDetection)(testWorkspacePath, 'xss-document-write.js', vulnerableCode);
...
`;
            const vulnerabilities = await (0, securityTestHelper_1.testXSSDetection)(testWorkspacePath, 'xss-outerHTML.js', vulnerableCode);
...
`;
            const vulnerabilities = await (0, securityTestHelper_1.testXSSDetection)(testWorkspacePath, 'xss-dom-hash.js', vulnerableCode);
...
`;
            const vulnerabilities = await (0, securityTestHelper_1.testXSSDetection)(testWorkspacePath, 'xss-window-location.js', vulnerableCode);
...
`;
            const vulnerabilities = await (0, securityTestHelper_1.testVulnerabilityDetection)(testWorkspacePath, 'xss-react-dangerous.jsx', vulnerableCode, securityTestConstants_1.VULNERABILITY_CATEGORIES.XSS, securityTestConstants_1.CWE_IDS.XSS, securityTestConstants_1.SEVERITY_LEVELS.HIGH, {
...
`;
            const vulnerabilities = await (0, securityTestHelper_1.testXSSDetection)(testWorkspacePath, 'xss-react-server.jsx', vulnerableCode);
...
`;
            await (0, securityTestHelper_1.testNoVulnerabilitiesDetected)(testWorkspacePath, 'xss-safe-textContent.js', safeCode, securityTestConstants_1.VULNERABILITY_CATEGORIES.XSS);
        });
...
`;
            await (0, securityTestHelper_1.testNoVulnerabilitiesDetected)(testWorkspacePath, 'xss-safe-sanitized.js', safeCode, securityTestConstants_1.VULNERABILITY_CATEGORIES.XSS);
        });
...
`;
            await (0, securityTestHelper_1.testNoVulnerabilitiesDetected)(testWorkspacePath, 'xss-safe-react.jsx', safeCode, securityTestConstants_1.VULNERABILITY_CATEGORIES.XSS);
        });
    });
...
`;
            const vulnerabilities = await (0, securityTestHelper_1.testXSSDetection)(testWorkspacePath, 'xss-multiple.js', vulnerableCode, { minVulnerabilities: 1 });
            for (const vuln of vulnerabilities) {
                assert.ok(vuln.owaspCategory?.includes('A03:2021'), `XSS vulnerability should map to OWASP A03:2021, got: ${vuln.owaspCategory}`);
            }
        });
...
`;
            const vulnerabilities = await (0, securityTestHelper_1.testXSSDetection)(testWorkspacePath, 'xss-cwe-validation.js', vulnerableCode);
            assert.strictEqual(vulnerabilities[0].cweId, securityTestConstants_1.CWE_IDS.XSS, 'XSS should have CWE-79');
        });
...
`;
            const vulnerabilities = await (0, securityTestHelper_1.testXSSDetection)(testWorkspacePath, 'xss-recommendation.js', vulnerableCode);
            const recommendation = vulnerabilities[0].recommendation.toLowerCase();
            assert.ok(recommendation.includes('sanitize') ||
                recommendation.includes('escape') ||
                recommendation.includes('textcontent'), `Recommendation should mention sanitization/escaping. Got: ${vulnerabilities[0].recommendation}`);
        });
...
`;
            const vulnerabilities = await (0, securityTestHelper_1.testXSSDetection)(testWorkspacePath, 'xss-references.js', vulnerableCode);
            assert.ok(vulnerabilities[0].references.length >= 2, 'Should have at least 2 references');
            assert.ok(vulnerabilities[0].references.some(ref => ref.includes('owasp.org')), 'Should have OWASP reference');
            assert.ok(vulnerabilities[0].references.some(ref => ref.includes('cwe.mitre.org')), 'Should have CWE reference');
        });
    });
});
//# sourceMappingURL=security.xss.test.js.map