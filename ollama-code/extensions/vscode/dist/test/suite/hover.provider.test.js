...
suite('Hover Provider Tests', () => {
    ...
    setup(async function () {
        ...
        mockClient = (0, providerTestHelper_1.createMockOllamaClient)(true, (0, providerTestHelper_1.createHoverAIHandler)());
        mockLogger = (0, providerTestHelper_1.createMockLogger)();
        // Create provider
        hoverProvider = new hoverProvider_1.HoverProvider(mockClient, mockLogger);
        // Create test workspace
        testWorkspacePath = await (0, extensionTestHelper_1.createTestWorkspace)('hover-provider-tests');
...
function calculateSum(a, b) {
  return a + b;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'function.ts', code);
            ...
            assert.ok(markdownContent.includes('Function') || markdownContent.includes('function'), 'Should identify as function');
            assert.ok(markdownContent.includes('calculateSum'), 'Should include function name');
        });
...
export function calculateSum(a, b) {
  return a + b;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'signature.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const position = new vscode.Position(0, 17); // Position on function name
            const hover = await hoverProvider.provideHover(document, position, cancellationToken);
            assert.ok(hover, 'Should provide hover');
            const markdownContent = hover.contents[0].value;
            assert.ok(markdownContent.includes('calculateSum'), 'Should include function signature');
        });
...
async function fetchData(url) {
  ...
  return response.json();
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'async.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const position = new vscode.Position(0, 20); // Position on "fetchData"
            const hover = await hoverProvider.provideHover(document, position, cancellationToken);
            assert.ok(hover, 'Should provide hover');
            const markdownContent = hover.contents[0].value;
            assert.ok(markdownContent.toLowerCase().includes('async'), 'Should indicate async function');
        });
...
function complex(x) {
  ...
  return x;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'complex.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const position = new vscode.Position(0, 10);
            const hover = await hoverProvider.provideHover(document, position, cancellationToken);
            assert.ok(hover, 'Should provide hover');
            const markdownContent = hover.contents[0].value;
            assert.ok(markdownContent.includes('Complexity') || markdownContent.includes('complexity'), 'Should show complexity information');
        });
    });
...
export class UserService {
  ...
  getUser(id) {
    return this.users.find(u => u.id === id);
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'class.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const position = new vscode.Position(0, 15); // Position on "UserService"
            const hover = await hoverProvider.provideHover(document, position, cancellationToken);
            assert.ok(hover, 'Should provide hover for class');
            const markdownContent = hover.contents[0].value;
            assert.ok(markdownContent.includes('Class') || markdownContent.includes('class'), 'Should identify as class');
            assert.ok(markdownContent.includes('UserService'), 'Should include class name');
        });
...
class Calculator {
  add(a, b) {
    return a + b;
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'method.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const position = new vscode.Position(1, 3); // Position on "add"
            const hover = await hoverProvider.provideHover(document, position, cancellationToken);
            assert.ok(hover, 'Should provide hover for method');
            const markdownContent = hover.contents[0].value;
            assert.ok(markdownContent.includes('Method') || markdownContent.includes('method') ||
                markdownContent.includes('Function') || markdownContent.includes('function'), 'Should identify as method or function');
        });
    });
...
const data = fetchData(API_URL);
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'const.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const position = new vscode.Position(0, 10); // Position on "API_URL"
            const hover = await hoverProvider.provideHover(document, position, cancellationToken);
            assert.ok(hover, 'Should provide hover for constant');
            const markdownContent = hover.contents[0].value;
            assert.ok(markdownContent.includes('API_URL') || markdownContent.toLowerCase().includes('variable'), 'Should show variable/constant information');
        });
...
user.getName();
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'property.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
...
export function publicApi() {
  return 'public';
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'export.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const position = new vscode.Position(0, 20);
            const hover = await hoverProvider.provideHover(document, position, cancellationToken);
            assert.ok(hover, 'Should provide hover');
            const markdownContent = hover.contents[0].value;
            assert.ok(markdownContent.includes('export'), 'Should indicate exported symbol');
        });
...
function testCalculation() {
  ...
  assert.equal(result, 10);
      ...
      `.trim();
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'test.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
            const position = new vscode.Position(0, 10);
            const hover = await hoverProvider.provideHover(document, position, cancellationToken);
            assert.ok(hover, 'Should provide hover');
            const markdownContent = hover.contents[0].value;
            assert.ok(markdownContent.toLowerCase().includes('test'), 'Should indicate test function');
        });
    });
...
def calculate_total(items):
    ...
    suite('Caching and Performance', () => {
        test('Should cache hover results', async function () {
            ...
            assert.strictEqual(hover1.contents[0].value, hover2.contents[0].value, 'Cached results should match');
        });
        test('Should handle timeout gracefully', async function () {
            ...
            const slowClient = {
                ...
                sendAIRequest: async () => {
                    await (0, extensionTestHelper_1.sleep)(5000); // 5 second delay - will timeout
                    return { result: 'too slow' };
                }
            };
            const slowProvider = new hoverProvider_1.HoverProvider(slowClient, mockLogger);
    ...
    suite('Error Handling', () => {
        test('Should return null when disconnected', async function () {
            ...
            const provider = new hoverProvider_1.HoverProvider(disconnectedClient, mockLogger);
            const code = 'function test() {}';
        ...
        test('Should return null for invalid positions', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const code = 'function test() {}';
            const filePath = await (0, extensionTestHelper_1.createTestFile)(testWorkspacePath, 'invalid.ts', code);
            const document = await (0, extensionTestHelper_1.openDocument)(filePath);
        ...
        test('Should handle AI request errors gracefully', async function () {
            ...
            const provider = new hoverProvider_1.HoverProvider(errorClient, mockLogger);
            const code = 'function test() {}';
