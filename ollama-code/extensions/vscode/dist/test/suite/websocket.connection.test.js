...
suite('WebSocket Connection Management Tests', () => {
    const TEST_PORT = test_constants_1.WEBSOCKET_TEST_CONSTANTS.PORTS.CONNECTION_TESTS;
    const WS_URL = `ws://localhost:${TEST_PORT}`;
    const VALID_AUTH_TOKEN = test_constants_1.WEBSOCKET_TEST_CONSTANTS.AUTH.VALID_TOKEN;
    let mockServer;
    let testClient;
    ...
    suite('Authentication', () => {
        test('Should connect successfully with valid authentication token', async function () {
            ...
            assert.strictEqual(mockServer.getConnectedClients(), 1, 'Client with valid auth token should connect successfully');
            assert.strictEqual(testClient.isConnected, true, 'Client should report connected state');
        });
        test('Should reject connection with invalid authentication token', async function () {
            ...
            try {
                ...
                assert.strictEqual(mockServer.getConnectedClients(), 0, 'Server should have 0 connected clients');
                assert.strictEqual(mockServer.getRejectedConnections(), 1, 'Server should have rejected 1 connection');
            }
        });
    ...
    suite('Multiple Concurrent Connections', () => {
        test('Should handle multiple concurrent client connections', async function () {
            ...
            const connectPromises = Array.from({ length: CLIENT_COUNT }, () => {
                const client = (0, websocketTestHelper_1.createTestWebSocketClient)(WS_URL);
                clients.push(client);
                return client.connect();
            });
            await Promise.all(connectPromises);
            assert.strictEqual(mockServer.getConnectedClients(), CLIENT_COUNT, `Server should have ${CLIENT_COUNT} connected clients`);
            // Verify all clients report connected state
            clients.forEach((client, index) => {
                assert.strictEqual(client.isConnected, true, `Client ${index + 1} should be connected`);
            });
            // Disconnect all clients
            await Promise.all(clients.map(client => client.disconnect()));
            await (0, test_utils_1.sleep)(test_constants_1.TEST_DELAYS.DISCONNECTION);
            assert.strictEqual(mockServer.getConnectedClients(), 0, 'All clients should be disconnected');
        });
        test('Should enforce connection limit and reject excess connections', async function () {
            ...
            assert.strictEqual(mockServer.getConnectedClients(), MAX_CONNECTIONS, 'Server should have reached connection limit');
            // Try to connect beyond limit
            const excessClient = (0, websocketTestHelper_1.createTestWebSocketClient)(WS_URL);
            try {
    ...
    suite('Disconnection Handling', () => {
        test('Should handle graceful client disconnection', async function () {
            ...
            // Graceful disconnect
            await testClient.disconnect();
            await (0, test_utils_1.sleep)(test_constants_1.TEST_DELAYS.CLEANUP);
            assert.strictEqual(mockServer.getConnectedClients(), 0, 'Server should have 0 clients after graceful disconnect');
            assert.strictEqual(testClient.isConnected, false, 'Client should report disconnected state');
        });
        test('Should handle abrupt client disconnection (terminate)', async function () {
            ...
            await (0, test_utils_1.sleep)(test_constants_1.TEST_DELAYS.DISCONNECTION);
            assert.strictEqual(mockServer.getConnectedClients(), 0, 'Server should detect abrupt disconnection');
        });
    });
    suite('Timeout and Reliability', () => {
        test('Should handle connection timeout for unreachable server', async function () {
            ...
            const unreachableClient = (0, websocketTestHelper_1.createTestWebSocketClient)('ws://localhost:9999', {
            ...
            try {
                ...
                assert.ok(elapsed >= 2000 && elapsed < 3000, `Should timeout after ~2 seconds (actual: ${elapsed}ms)`);
                assert.ok(error instanceof Error && error.message.includes('timeout'), 'Error should mention timeout');
            }
        });
        test('Should maintain connection stability over time', async function () {
            ...
            assert.strictEqual(testClient.isConnected, true, 'Client should remain connected');
            assert.strictEqual(testClient.errors.length, 0, 'Client should have no errors');
            assert.strictEqual(mockServer.getReceivedMessages().length, 10, 'Server should receive all messages');
        });
    });
    suite('Heartbeat and Keep-Alive', () => {
        test('Should support heartbeat/ping-pong mechanism', async function () {
            ...
            await (0, test_utils_1.sleep)(3500);
            assert.ok(pingCount >= 2, `Should receive at least 2 pings (received ${pingCount})`);
            assert.strictEqual(testClient.isConnected, true, 'Connection should remain alive');
        });
        test('Should respond to heartbeat ping messages', async function () {
            ...
            assert.strictEqual(response.type, 'pong', 'Server should respond with pong message');
            assert.ok(response.timestamp, 'Pong response should include timestamp');
        });
    });
    suite('Reconnection Logic', () => {
        test('Should successfully reconnect after network interruption', async function () {
            ...
            // Reconnect
            await testClient.connect();
            assert.strictEqual(testClient.isConnected, true, 'Client should reconnect successfully');
            assert.strictEqual(mockServer.getConnectedClients(), 1, 'Server should have 1 connected client after reconnection');
        });
        test('Should handle rapid reconnection attempts', async function () {
            ...
            for (let i = 0; i < 5; i++) {
                const client = (0, websocketTestHelper_1.createTestWebSocketClient)(WS_URL);
                await client.connect();
                assert.strictEqual(client.isConnected, true, `Cycle ${i + 1}: Should connect successfully`);
                await client.send({ type: 'test', cycle: i });
                await client.disconnect();
                assert.strictEqual(client.isConnected, false, `Cycle ${i + 1}: Should disconnect successfully`);
                await (0, test_utils_1.sleep)(test_constants_1.TEST_DELAYS.CLEANUP);
            }
            const messages = mockServer.getReceivedMessages();
            assert.strictEqual(messages.length, 5, 'Server should receive all messages from reconnection cycles');
        });
    });
    suite('SSL/TLS Security (Mock)', () => {
        test('Should accept secure WebSocket connections (wss://)', async function () {
            ...
            assert.strictEqual(testClient.isConnected, true, 'Should establish connection (ws:// for testing)');
        ...
        test('Should include custom headers for security', async function () {
            ...
            await testClient.connect();
            assert.strictEqual(testClient.isConnected, true, 'Should connect with custom headers');
        });
    });
    suite('Edge Cases', () => {
        test('Should handle connection when server is at capacity and slot opens', async function () {
            ...
            // Fill to capacity
            const client1 = (0, websocketTestHelper_1.createTestWebSocketClient)(WS_URL);
            const client2 = (0, websocketTestHelper_1.createTestWebSocketClient)(WS_URL);
            await client1.connect();
            await client2.connect();
            assert.strictEqual(mockServer.getConnectedClients(), MAX_CONNECTIONS);
            // Try to connect - should fail
            const client3 = (0, websocketTestHelper_1.createTestWebSocketClient)(WS_URL);
            try {
            ...
            await client3.connect();
            assert.strictEqual(client3.isConnected, true, 'Should connect after slot opens');
            assert.strictEqual(mockServer.getConnectedClients(), MAX_CONNECTIONS, 'Should maintain connection limit');
