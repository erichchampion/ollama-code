...
suite('Security - Authentication & Session Issues Tests', () => {
    ...
    suite('Hardcoded Credentials Detection', () => {
        test('Should detect hardcoded password in assignment', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const vulnerableCode = securityTestConstants_1.VULNERABILITY_CODE_TEMPLATES.AUTHENTICATION.HARDCODED_PASSWORD();
            ...
            assert.ok(vulnerabilities[0].owaspCategory?.includes(securityTestConstants_1.OWASP_CATEGORIES.A07_AUTHENTICATION));
        ...
        test('Should detect hardcoded API key', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const vulnerableCode = securityTestConstants_1.VULNERABILITY_CODE_TEMPLATES.AUTHENTICATION.HARDCODED_API_KEY();
        ...
        test('Should NOT detect credentials from environment variables', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const safeCode = securityTestConstants_1.VULNERABILITY_CODE_TEMPLATES.AUTHENTICATION.SAFE_ENV_VARS();
    ...
    suite('Weak Password Policy Detection', () => {
        test('Should detect password length less than 8 characters', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const vulnerableCode = securityTestConstants_1.VULNERABILITY_CODE_TEMPLATES.AUTHENTICATION.WEAK_PASSWORD_LENGTH();
        ...
        test('Should detect weak minLength configuration', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const vulnerableCode = securityTestConstants_1.VULNERABILITY_CODE_TEMPLATES.AUTHENTICATION.WEAK_MIN_LENGTH_CONFIG();
        ...
        test('Should NOT detect strong password policy (>= 8 characters)', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const safeCode = securityTestConstants_1.VULNERABILITY_CODE_TEMPLATES.AUTHENTICATION.SAFE_STRONG_PASSWORD();
    ...
    suite('Missing Authentication Check Detection', () => {
        test('Should detect unprotected admin route', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const vulnerableCode = securityTestConstants_1.VULNERABILITY_CODE_TEMPLATES.AUTHENTICATION.UNPROTECTED_ADMIN_ROUTE();
            ...
            assert.strictEqual(vulnerabilities[0].severity, securityTestConstants_1.SEVERITY_LEVELS.CRITICAL);
            assert.ok(vulnerabilities[0].owaspCategory?.includes(securityTestConstants_1.OWASP_CATEGORIES.A01_ACCESS_CONTROL));
        });
        test('Should detect unprotected API endpoint', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const vulnerableCode = securityTestConstants_1.VULNERABILITY_CODE_TEMPLATES.AUTHENTICATION.UNPROTECTED_API_ENDPOINT();
        ...
        test('Should NOT detect protected route with authentication middleware', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const safeCode = securityTestConstants_1.VULNERABILITY_CODE_TEMPLATES.AUTHENTICATION.SAFE_PROTECTED_ROUTE();
    ...
    suite('Session Fixation Detection', () => {
        test('Should detect session fixation in login without regenerate', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const vulnerableCode = securityTestConstants_1.VULNERABILITY_CODE_TEMPLATES.AUTHENTICATION.SESSION_FIXATION();
        ...
        test('Should NOT detect session with proper regeneration', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const safeCode = securityTestConstants_1.VULNERABILITY_CODE_TEMPLATES.AUTHENTICATION.SAFE_SESSION_REGENERATE();
...
`;
            ...
            for (const vuln of vulnerabilities) {
                assert.ok(vuln.owaspCategory?.includes(securityTestConstants_1.OWASP_CATEGORIES.A07_AUTHENTICATION) ||
                    vuln.owaspCategory?.includes(securityTestConstants_1.OWASP_CATEGORIES.A01_ACCESS_CONTROL), `Auth vulnerability should map to OWASP A07:2021 or A01:2021, got: ${vuln.owaspCategory}`);
            }
        });
...
`;
    ...
    suite('Edge Case Tests', () => {
        test('Should detect numeric-only hardcoded secrets', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const vulnerableCode = securityTestConstants_1.VULNERABILITY_CODE_TEMPLATES.AUTHENTICATION.HARDCODED_NUMERIC();
            const vulnerabilities = await (0, securityTestHelper_1.testHardcodedCredentialsDetection)(testWorkspacePath, 'auth-numeric-secrets.js', vulnerableCode);
        ...
        test('Should detect session fixation with regenerate in comment', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const vulnerableCode = securityTestConstants_1.VULNERABILITY_CODE_TEMPLATES.AUTHENTICATION.SESSION_FIXATION_COMMENT();
            const vulnerabilities = await (0, securityTestHelper_1.testSessionFixationDetection)(testWorkspacePath, 'auth-session-fixation-comment.js', vulnerableCode);
        ...
        test('Should detect password length boundary (exactly 7 characters)', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const vulnerableCode = securityTestConstants_1.VULNERABILITY_CODE_TEMPLATES.AUTHENTICATION.WEAK_PASSWORD_LENGTH(7);
            const vulnerabilities = await (0, securityTestHelper_1.testWeakPasswordPolicyDetection)(testWorkspacePath, 'auth-boundary-password.js', vulnerableCode);
        ...
        test('Should NOT detect password length boundary (exactly 8 characters)', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const vulnerableCode = securityTestConstants_1.VULNERABILITY_CODE_TEMPLATES.AUTHENTICATION.WEAK_PASSWORD_LENGTH(8);
            await (0, securityTestHelper_1.testNoVulnerabilitiesDetected)(testWorkspacePath, 'auth-boundary-safe-password.js', vulnerableCode, securityTestConstants_1.VULNERABILITY_CATEGORIES.AUTHENTICATION);
        });
        test('Should detect very short hardcoded password (6 characters)', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const vulnerableCode = securityTestConstants_1.VULNERABILITY_CODE_TEMPLATES.AUTHENTICATION.HARDCODED_PASSWORD('abc123');
            const vulnerabilities = await (0, securityTestHelper_1.testHardcodedCredentialsDetection)(testWorkspacePath, 'auth-short-password.js', vulnerableCode);
