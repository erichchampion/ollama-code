...
suite('WebSocket Integration Tests', () => {
    const TEST_PORT = test_constants_1.WEBSOCKET_TEST_CONSTANTS.DEFAULT_TEST_PORT;
    ...
    test('WebSocket client should connect to mock server', async function () {
        ...
        assert.strictEqual(mockServer.getConnectedClients(), 1, 'Server should have 1 connected client');
    });
    test('WebSocket client should disconnect cleanly', async function () {
        ...
        assert.strictEqual(mockServer.getConnectedClients(), 0, 'Server should have 0 connected clients after disconnect');
    });
    test('WebSocket should send and receive JSON messages', async function () {
        ...
        assert.deepStrictEqual(receivedMessages[0], testMessage, 'Received message should match sent message');
    });
    test('WebSocket should handle broadcast messages from server', async function () {
        ...
        assert.deepStrictEqual(receivedMessage, broadcastMessage, 'Client should receive broadcast message');
    });
    test('WebSocket should handle multiple clients', async function () {
        ...
        assert.strictEqual(mockServer.getConnectedClients(), 2, 'Server should have 2 connected clients');
    ...
    test('WebSocket should handle connection timeout gracefully', async function () {
        ...
        const invalidClient = (0, websocketTestHelper_1.createTestWebSocketClient)('ws://localhost:9999');
        try {
    ...
    test('WebSocket should handle malformed JSON gracefully', async function () {
        ...
        assert.strictEqual(receivedMessages[0], malformedMessage, 'Server should receive malformed message as string');
    });
    test('WebSocket should maintain connection stability', async function () {
        ...
        const receivedMessages = mockServer.getReceivedMessages();
        assert.strictEqual(receivedMessages.length, 10, 'Server should receive all 10 messages');
        // Verify sequence
        for (let i = 0; i < 10; i++) {
            assert.strictEqual(receivedMessages[i].sequence, i, `Message ${i} should have correct sequence number`);
        }
    });
    test('WebSocket should handle rapid connect/disconnect cycles', async function () {
        ...
        for (let i = 0; i < 5; i++) {
            const cycleClient = (0, websocketTestHelper_1.createTestWebSocketClient)(WS_URL);
            await cycleClient.connect();
            await cycleClient.send({ type: 'test', cycle: i });
            await cycleClient.disconnect();
        }
        const receivedMessages = mockServer.getReceivedMessages();
        assert.strictEqual(receivedMessages.length, 5, 'Server should receive all messages from connect/disconnect cycles');
    });
    test('WebSocket client should track errors correctly', async function () {
        this.timeout(3000);
        // Try to connect to non-existent server
        const errorClient = (0, websocketTestHelper_1.createTestWebSocketClient)('ws://localhost:9998');
        try {
        ...
        assert.ok(errorClient.errors.length > 0, 'Client should track connection errors');
    });
    test('WebSocket should handle large message payloads', async function () {
        ...
        const receivedMessages = mockServer.getReceivedMessages();
        assert.strictEqual(receivedMessages.length, 1, 'Server should receive large message');
        assert.strictEqual(receivedMessages[0].data.length, largeData.length, 'Large message data should be intact');
    });
});
