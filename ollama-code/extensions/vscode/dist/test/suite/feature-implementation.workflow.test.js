...
const featureImplementationWrapper_1 = require("../helpers/featureImplementationWrapper");
suite('Feature Implementation Workflow Tests', () => {
    ...
    setup(async function () {
        ...
        testWorkspacePath = await (0, extensionTestHelper_1.createTestWorkspace)('feature-implementation-tests');
    });
    ...
    suite('Specification Parsing', () => {
        test('Should parse text specification to structured requirements', async function () {
            this.timeout(test_constants_1.PROVIDER_TEST_TIMEOUTS.STANDARD_TEST);
            const spec = {
                text: test_constants_1.FEATURE_IMPLEMENTATION_CONSTANTS.MODERATE_SPEC_TEXT,
                type: 'text',
            };
            const workflow = new featureImplementationWrapper_1.FeatureImplementationWorkflow();
            ...
            assert.ok(requirements.every((r) => r.acceptanceCriteria.length > 0), 'All requirements should have acceptance criteria');
        });
        ...
        test('Should perform complexity analysis (simple, moderate, complex)', async function () {
            ...
            // Test simple feature
            const simpleSpec = {
                text: test_constants_1.FEATURE_IMPLEMENTATION_CONSTANTS.SIMPLE_SPEC_TEXT,
                type: 'text',
            };
            ...
            const complexSpec = {
                text: test_constants_1.FEATURE_IMPLEMENTATION_CONSTANTS.COMPLEX_SPEC_TEXT,
                type: 'text',
            };
            ...
            assert.ok(complexComplexity.level === 'complex' || complexComplexity.level === 'very_complex', 'Complex feature should be rated complex or very_complex');
        ...
        test('Should estimate implementation time', async function () {
            ...
            const sum = timeEstimation.breakdown.design +
        ...
        test('Should identify technical challenges from specification', async function () {
            ...
            assert.ok(challengesText.includes('real-time') ||
                challengesText.includes('security') ||
                challengesText.includes('performance'), 'Should identify real-time, security, or performance challenges');
        });
        test('Should provide complexity justification', async function () {
            ...
            assert.ok(complexity.justification.includes(complexity.level), 'Justification should mention complexity level');
    ...
    suite('Implementation Decomposition', () => {
        ...
        test('Should identify critical path through tasks', async function () {
            ...
            assert.ok(plan.criticalPath.length >= plan.phases.length - 1, 'Critical path should span most phases');
        });
        test('Should assess risks with mitigation strategies', async function () {
            ...
            const workflow = new featureImplementationWrapper_1.FeatureImplementationWorkflow({ includeRiskAssessment: true });
            const plan = await workflow.generatePlan(spec);
        ...
        test('Should optimize execution order based on dependencies', async function () {
            ...
            for (let i = 0; i < allTasks.length; i++) {
                ...
                for (const depId of task.dependencies) {
                    ...
                    if (depIndex !== -1) {
                        assert.ok(depIndex < i, `Dependency ${depId} should come before task ${task.id} in execution order`);
                    }
                }
            }
        });
        ...
        test('Should estimate task durations that sum to phase durations', async function () {
            ...
            for (const phase of plan.phases) {
                ...
                assert.ok(Math.abs(taskHoursSum - phase.duration) <= tolerance, `Phase ${phase.number} task hours (${taskHoursSum}) should approximately equal phase duration (${phase.duration})`);
            }
        });
        ...
        test('Should calculate resource allocation correctly', async function () {
            ...
            assert.strictEqual(totalRoleHours, plan.resources.totalPersonHours, 'Total person-hours should match sum of role allocations');
        });
        test('Should handle configuration options (maxPhases, includeRiskAssessment)', async function () {
            ...
            const workflowNoRisk = new featureImplementationWrapper_1.FeatureImplementationWorkflow({ includeRiskAssessment: false });
            ...
            const workflowWithRisk = new featureImplementationWrapper_1.FeatureImplementationWorkflow({ includeRiskAssessment: true });
...
//# sourceMappingURL=feature-implementation.workflow.test.js.map