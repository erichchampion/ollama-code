...
export class IDEIntegrationServer extends EventEmitter {
    ...
    startTime = null;
    constructor(port = IDE_SERVER_DEFAULTS.PORT) {
        super();
        this.port = port;
     ...
     */
    async handleAIRequest(client, request) {
        const aiRequest = request.payload;
        ...
        try {
            ...
            switch (aiRequest.type) {
                case 'completion':
                    ...
                    if (aiRequest.language)
                        args.push('--language', aiRequest.language);
                    break;
                case 'explanation':
                    command = 'explain';
            ...
            this.sendError(client, request.id, getErrorMessage(error));
        }
    }
    /**
     * Handle direct command execution requests
     */
    async handleCommandRequest(client, request) {
        ...
        try {
            ...
            this.sendError(client, request.id, `Command execution failed: ${getErrorMessage(error)}`);
        }
    }
    /**
     * Handle workspace context updates from IDE
     */
    async handleContextUpdate(client, request) {
     ...
     */
    async handleWorkspaceAnalysis(client, request) {
        ...
        try {
            ...
            this.sendError(client, request.id, `Workspace analysis failed: ${getErrorMessage(error)}`);
        }
    }
    /**
     ...
     */
    sendProgress(client, requestId, message, progress) {
        this.sendResponse(client, {
            id: requestId,
            type: 'progress',
 ...
 */
let ideIntegrationServer = null;
/**
 * Get or create IDE integration server instance
 */
export function getIDEIntegrationServer(port) {
 ...
 */
export async function startIDEIntegrationService(port) {
 ...
 */
export async function stopIDEIntegrationService() {
    if (ideIntegrationServer) {
        await ideIntegrationServer.stop();
        ideIntegrationServer = null;
    }
}
//# sourceMappingURL=integration-server.js.map