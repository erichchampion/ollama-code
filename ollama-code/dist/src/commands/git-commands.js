 ...
 */
function registerGitStatusCommand() {
    const command = {
        name: 'git-status',
 ...
 */
function registerGitCommitCommand() {
    const command = {
        name: 'git-commit',
 ...
 */
function registerGitBranchCommand() {
    const command = {
        name: 'git-branch',
        ...
        async handler(args) {
            try {
                ...
                if (action === 'list' || !action) {
                    ...
                    await executeWithSpinner(`Creating branch: ${name}`, async () => {
                    ...
                    await executeWithSpinner(`Switching to branch: ${name}`, async () => {
                        const { exec } = await import('child_process');
                        const { promisify } = await import('util');
                        const execAsync = promisify(exec);
 ...
 */
function registerGitConflictCommand() {
    const command = {
        name: 'git-conflicts',
 ...
 */
function registerGitInitCommand() {
    const command = {
        name: 'git-init',
 ...
 */
function formatCommitSuggestionDisplay(suggestion) {
