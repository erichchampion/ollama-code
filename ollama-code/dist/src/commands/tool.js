...
async function handleListTools(category) {
...
async function handleDescribeTool(toolName) {
...
async function handleExecuteTool(toolName, parametersJson, options = {}) {
    const tool = toolRegistry.get(toolName);
    if (!tool) {
        throw new Error(`Tool '${toolName}' not found`);
    }
...
async function handleOrchestrate(planJson, options = {}) {
    if (!planJson) {
