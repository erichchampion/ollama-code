...
`;
     ...
     */
    async getCommandCompletions(context) {
     ...
     */
    async getArgumentCompletions(context) {
     ...
     */
    async getOptionCompletions(context) {
        if (!context.commandName)
            return [];
        const command = commandRegistry.get(context.commandName);
        if (!command || !command.args)
     ...
     */
    async getFileCompletions(context) {
        const completions = [];
        try {
     ...
     */
    async getModelCompletions(context) {
     ...
     */
    async getBranchCompletions(context) {
        const completions = [];
        try {
     ...
     */
    async getContextualCompletions(context) {
        const completions = [];
     ...
     */
    async getGitFileCompletions(context) {
        const completions = [];
        try {
            const { exec } = await import('child_process');
            const { promisify } = await import('util');
     ...
     */
    deduplicateCompletions(completions) {
        const seen = new Set();
