    ...
    async installHooks(options) {
        try {
            ...
            if (config.enablePreCommit)
                console.log('  • pre-commit: AI-powered code analysis');
            if (config.enableCommitMsg)
                console.log('  • commit-msg: Intelligent commit message enhancement');
            if (config.enablePrePush)
                console.log('  • pre-push: Regression risk assessment');
            if (config.enablePostMerge)
                console.log('  • post-merge: Quality trend tracking');
            console.log('');
 ...
 */
export function createGitHooksCLI() {
    return new GitHooksCLI();
}
/**
 * Execute git hooks CLI with provided arguments
 */
export async function executeGitHooksCLI(argv) {
    const cli = createGitHooksCLI();
    await cli.parse(argv);
}
//# sourceMappingURL=git-hooks-cli.js.map