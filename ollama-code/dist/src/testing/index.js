...
export class TestManager {
     ...
     */
    async setupFramework(frameworkName) {
...
Return only the test code, ready to save as a .test.${filePath.endsWith('.ts') ? 'ts' : 'js'} file.`;
    ...
    async runTests(pattern) {
        ...
        try {
            ...
            if (error instanceof Error && 'stdout' in error) {
                const framework = await this.detectFramework();
                if (framework) {
                    return this.parseTestResults(error.stdout, error.stderr, framework.name);
                }
            }
            throw error;
        }
     ...
     */
    async suggestTestImprovements(testFilePath) {
...
Provide specific, actionable suggestions.`;
     ...
     */
    async analyzeCodeStructure(code, filePath) {
        const analysis = {
            exports: [],
            functions: [],
            classes: [],
...
`;
     ...
     * Update package.json scripts
     */
    async updatePackageJsonScripts(framework) {
        const packageJsonPath = path.join(this.workingDir, 'package.json');
        if (!await fileExists(packageJsonPath)) {
            return;
        }
     ...
     * Create test directory structure
     */
    async createTestDirectories(framework) {
        const testDirs = [
            'src/__tests__',
            'tests/unit',
            'tests/integration'
     ...
     * Parse coverage report
     */
    parseCoverageReport(output, framework) {
        try {
