...
export class LazyProjectContext extends ProjectContext {
    ...
    async getFiles(pattern) {
        ...
        try {
            ...
            await this.ensureInitialized();
            return this.allFiles.filter(file => file.relativePath.includes(pattern) || file.path.includes(pattern));
        }
    }
    /**
    ...
    async findFilesByExtension(extensions) {
        ...
        try {
            ...
            const fileInfos = await Promise.all(allFiles.slice(0, CONSTANTS.MAX_ERROR_CONTEXT_LINES * 20).map(async (file) => {
                try {
        ...
        try {
            ...
            if (dirs.length > 0) {
                ...
                try {
                    ...
                    const sampleEntries = await fs.readdir(path.join(this.root, sampleDir.name), { withFileTypes: true });
...
//# sourceMappingURL=lazy-project-context.js.map