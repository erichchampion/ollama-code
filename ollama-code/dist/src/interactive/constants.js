 ...
 */
export function getMemoryCategory(memoryMB) {
    if (memoryMB < 100)
        return MEMORY_CATEGORIES.MINIMAL;
    if (memoryMB < 250)
        return MEMORY_CATEGORIES.LIGHT;
    if (memoryMB < 500)
        return MEMORY_CATEGORIES.MODERATE;
    if (memoryMB < 1024)
        return MEMORY_CATEGORIES.HEAVY;
    return MEMORY_CATEGORIES.EXCESSIVE;
}
/**
 * Get performance category based on duration
 */
export function getPerformanceCategory(durationMs) {
    if (durationMs < 1000)
        return PERFORMANCE_CATEGORIES.EXCELLENT;
    if (durationMs < 3000)
        return PERFORMANCE_CATEGORIES.GOOD;
    if (durationMs < 10000)
        return PERFORMANCE_CATEGORIES.ACCEPTABLE;
    if (durationMs < 30000)
        return PERFORMANCE_CATEGORIES.SLOW;
...
export function validateTimeout(timeoutMs) {
    return Math.max(CONSTANTS.MIN_TIMEOUT_MS, Math.min(timeoutMs, CONSTANTS.MAX_TIMEOUT_MS));
}
/**
 * Validate retry count
 */
export function validateRetryCount(retries) {
    return Math.max(CONSTANTS.MIN_RETRY_COUNT, Math.min(retries, CONSTANTS.MAX_RETRY_COUNT));
}
/**
