...
export const TIMEOUT_CONFIG = {
    ...
    INTENT_ANALYZER: 3000, // 3 seconds
    CONVERSATION_MANAGER: 1000, // 1 second (no async init)
    TASK_PLANNER: 5000, // 5 seconds
    ...
    NATURAL_LANGUAGE_ROUTER: 3000, // 3 seconds
 ...
 */
const COMPONENT_TIMEOUT_MAP = new Map([
    // Direct mappings
    ['aiClient', 'AI_CLIENT'],
...
export function getRetryDelay(attempt) {
    return Math.min(TIMEOUT_CONFIG.SERVICE_RETRY_DELAY_BASE * Math.pow(2, attempt), TIMEOUT_CONFIG.SERVICE_RETRY_DELAY_MAX);
}
/**
 * Create a timeout promise that can be cancelled
 */
export function createCancellableTimeout(timeoutMs, errorMessage) {
    let timeoutHandle;
