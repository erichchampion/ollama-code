 ...
 */
export async function validateCompilation(artifacts, context) {
    try {
        // In a real implementation, this would run actual TypeScript compilation
        // For now, we simulate the check
        const hasTypeScriptFiles = artifacts.some(path => path.endsWith('.ts') || path.endsWith('.tsx'));
 ...
 */
export async function validateTestCoverage(requiredCoverage, artifacts, context) {
    try {
        ...
        const testFiles = artifacts.filter(path => path.includes('test') || path.includes('spec'));
        const sourceFiles = artifacts.filter(path => (path.endsWith('.ts') || path.endsWith('.js')) &&
 ...
 */
export async function validateCodeQuality(requiredScore, artifacts, context) {
    try {
        ...
        const hasDocumentation = artifacts.some(path => path.endsWith('.md') || path.includes('doc'));
        if (hasDocumentation)
            qualityScore += 0.05;
        // Check for configuration files
        const hasConfig = artifacts.some(path => path.includes('config') || path.endsWith('.json'));
        if (hasConfig)
            qualityScore += 0.02;
 ...
 */
export async function validatePerformanceThresholds(thresholds, artifacts, context) {
    try {
        ...
        return {
            ...
            details: allPassed
                ...
                : `Performance thresholds failed: ${failedMetrics.map(m => `${m.metric} (${m.actual}${m.unit} > ${m.threshold}${m.unit})`).join(', ')}`,
            context: {
                ...context,
 ...
 */
export async function validateSecurityChecks(checks, artifacts, context) {
    try {
 ...
 */
export async function validateTaskCriteria(criteria, artifacts, context) {
    const results = [];
 ...
 */
export function allValidationsPassed(results) {
 ...
 */
export function validateConfiguration(config, requiredFields, optionalFields) {
    ...
    if (missingFields.length > 0) {
        return errFromString(`Missing required configuration fields: ${missingFields.join(', ')}`, 'MISSING_CONFIG_FIELDS', { missingFields, providedFields: Object.keys(config) });
    }
    // Validate field types (basic validation)
    for (const [key, value] of Object.entries(config)) {
        if (value === null || value === undefined) {
            if (requiredFields.includes(key)) {
                return errFromString(`Required field '${key}' cannot be null or undefined`, 'NULL_REQUIRED_FIELD', { field: key });
            }
        }
    }
    return ok(config);
 ...
 */
export function createDefaultValidationCriteria(includeTestCoverage = true, includeCodeQuality = true, includeCompileCheck = true) {
    return {
