        ...
        try {
            ...
            if (options.mustExist !== false) {
                ...
                if (options.maxSize !== undefined && stats.size > options.maxSize) {
                    return {
                        isValid: false,
                        error: createValidationError('fileSize', stats.size, `file smaller than ${options.maxSize} bytes`)
                    };
                }
            }
            ...
            if (options.allowedExtensions) {
                ...
                if (!options.allowedExtensions.includes(ext)) {
                    return {
                        isValid: false,
                        error: createValidationError('fileExtension', ext, `one of: ${options.allowedExtensions.join(', ')}`)
                    };
                }
            }
            return {
...
export class StringValidator {
     ...
     */
    static validate(value, fieldName, minLength = 0, maxLength) {
        // Type check
        ...
        if (str.length < minLength) {
            return {
                isValid: false,
                error: createValidationError(fieldName, str, `string with at least ${minLength} characters`)
            };
        }
        if (maxLength && str.length > maxLength) {
            return {
                isValid: false,
                error: createValidationError(fieldName, str, `string with at most ${maxLength} characters`)
            };
        }
        return {
            isValid: true,
