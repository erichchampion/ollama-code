 ...
 */
export function padString(text, width, padChar = ' ', padRight = true) {
 ...
 */
export function centerString(text, width, padChar = ' ') {
    if (text.length >= width) {
        return text;
    }
 ...
 */
export function createTextTable(rows, headers) {
    if (rows.length === 0) {
        return '';
    }
 ...
 */
export function formatKeyValue(obj, options = {}) {
    const { indent = 0, keyValueSeparator = ': ', includeEmpty = false } = options;
