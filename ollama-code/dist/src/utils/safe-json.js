...
export function safeStringify(obj, options = {}) {
    const { maxDepth = 10, includeNonEnumerable = false, replacer, space } = options;
    const seen = new Set();
    const circularReplacer = (key, value, currentDepth = 0) => {
        // Handle depth limit
        ...
        try {
            ...
            if (Array.isArray(value)) {
                result = value.map((item, index) => circularReplacer(index.toString(), item, currentDepth + 1));
            }
            else {
                result = {};
 ...
 */
export function safeStringifyWithLimit(obj, maxSizeBytes = 1024 * 1024, // 1MB default
options = {}) {
 ...
 */
export function safeErrorStringify(error) {
