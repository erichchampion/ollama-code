...
import { DEFAULT_MEMORY_BUDGET, DEFAULT_CACHE_PRELOADING, BACKGROUND_PRELOAD_DELAY, DEFAULT_MAX_LISTENERS, DEFAULT_MAX_PRELOAD_TIME, DEFAULT_PARALLEL_PRELOADS, CACHE_LOAD_DELAY, INDEX_LOAD_DELAY } from '../constants/startup.js';
...
export class CachePreloader extends ManagedEventEmitter {
    ...
    async preloadCaches() {
        ...
        const cachesToPreload = Array.from(this.cacheRegistry.values())
            .filter(cache => this.strategy.preloadPriority.includes(cache.priority) &&
            !cache.preloaded)
            .sort((a, b) => {
            // Priority order
     ...
     */
    updateUsagePattern(key, accessTime) {
...
let globalCachePreloader = null;
/**
 * Initialize global cache preloader
 */
export async function initializeCachePreloader(strategy) {
 ...
 */
function registerDefaultCaches(preloader) {
    // Command cache
 ...
 */
function registerDefaultIndexes(preloader) {
    // File index
 ...
 */
export function getCachePreloader() {
    return globalCachePreloader;
}
/**
 * Start background cache preloading
 */
export async function startBackgroundPreloading() {
    setTimeout(async () => {
        try {
...
//# sourceMappingURL=cache-preloader.js.map