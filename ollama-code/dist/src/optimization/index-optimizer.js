...
import { DEFAULT_INDEX_CACHE_SIZE, CORE_MODULE_MEMORY } from '../constants/startup.js';
/**
...
export class IndexOptimizer {
     ...
     */
    async getOptimizedModule(modulePath) {
        // Record access pattern
    ...
    getModulePriority(modulePath) {
        if (modulePath.includes('logger') || modulePath.includes('core'))
            return 'critical';
        if (modulePath.includes('commands') || modulePath.includes('ai'))
            return 'high';
        if (modulePath.includes('optimization'))
            return 'normal';
        return 'lazy';
    }
    recordAccess(path) {
