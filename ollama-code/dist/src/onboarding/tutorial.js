...
export class TutorialSystem {
    ...
    async startTutorial(tutorialId) {
        ...
        if (tutorial.prerequisites) {
            const missingPrereqs = tutorial.prerequisites.filter(prereq => !this.onboardingState?.tutorials[prereq]?.completedAt);
    ...
    async showProgress() {
        ...
        const categories = this.groupTutorialsByCategory();
        for (const [category, tutorials] of categories.entries()) {
            const categoryCompleted = tutorials.filter(t => this.onboardingState?.tutorials[t.id]?.status === 'completed').length;
     ...
     */
    async resetProgress(tutorialId) {
        if (tutorialId) {
     ...
     * Run tutorial steps
     */
    async runTutorialSteps(tutorial, progress) {
     ...
     * Check for achievements
     */
    async checkAchievements(tutorial, progress, step) {
        ...
        const completedBeginner = beginnerTutorials.filter(t => this.onboardingState?.tutorials[t.id]?.status === 'completed');
     ...
     * Suggest next tutorials
     */
    async suggestNextTutorials(completedTutorial) {
        const nextTutorials = Array.from(this.availableTutorials.values())
            .filter(t => t.prerequisites?.includes(completedTutorial.id) &&
