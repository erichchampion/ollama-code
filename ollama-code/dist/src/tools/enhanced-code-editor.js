...
(function (ValidationLevel) {
    ValidationLevel["SYNTAX_ONLY"] = "syntax";
    ValidationLevel["SEMANTIC"] = "semantic";
    ValidationLevel["FULL_PROJECT"] = "full";
...
export class EnhancedCodeEditor extends CodeEditor {
     ...
     */
    async generateFileContent(fileOp, context) {
        if (fileOp.templateData && fileOp.action.parameters.templateId) {
            return await this.applyTemplate(fileOp.action.parameters.templateId, fileOp.templateData, context);
        }
     ...
     */
    async performFindReplace(fileOp) {
     ...
     */
    async performASTRefactor(fileOp) {
     ...
     */
    async applyTemplate(templateId, data, context) {
...
export const {{routeName}} = async (req: Request, res: Response): Promise<void> => {
  try {
     ...
     */
    enhanceEditResult(baseResult, fileOp) {
        return {
            ...baseResult,
            affectedFiles: [fileOp.path],
     ...
     */
    async validateEditRequest(request) {
        const errors = [];
        const warnings = [];
        // Basic validation
     ...
     */
    async generateEditPreview(request) {
        const previews = [];
        for (const fileOp of request.files) {
            try {
                const currentContent = await fs.readFile(fileOp.path, 'utf-8').catch(() => '');
    ...
    applyCodingStyle(content, style) {
    ...
    async assessCodeQuality(filePath) {
    ...
    async assessImpact(fileOp, context) {
     ...
     */
    getTemplatesByContext(language, framework) {
...
//# sourceMappingURL=enhanced-code-editor.js.map