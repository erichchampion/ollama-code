...
import { SECURITY_PATTERNS, FILE_SIZE_THRESHOLDS, FILE_CATEGORIES } from './safety-constants.js';
/**
 ...
 */
export function isSystemFile(filePath) {
...
export function isConfigFile(filePath) {
    ...
    return SECURITY_PATTERNS.CONFIG_FILES.some(pattern => pattern.test(fileName)) ||
        FILE_CATEGORIES.CONFIG.includes(extension);
}
/**
 * Check if file is a security-related file (keys, certificates, etc.)
 */
export function isSecurityFile(filePath) {
    const fileName = path.basename(filePath).toLowerCase();
    const fullPath = filePath.toLowerCase();
    return SECURITY_PATTERNS.SECURITY_FILES.some(pattern => pattern.test(fileName) || pattern.test(fullPath));
}
/**
 * Categorize file size
 */
export function categorizeFileSize(sizeBytes) {
    if (sizeBytes <= FILE_SIZE_THRESHOLDS.SMALL_FILE)
        return 'small';
    if (sizeBytes <= FILE_SIZE_THRESHOLDS.MEDIUM_FILE)
        return 'medium';
    if (sizeBytes <= FILE_SIZE_THRESHOLDS.LARGE_FILE)
        return 'large';
    return 'huge';
}
/**
...
export function getFileCategory(filePath) {
    const extension = path.extname(filePath).toLowerCase();
    if (FILE_CATEGORIES.SOURCE_CODE.includes(extension))
        return 'source_code';
    if (FILE_CATEGORIES.CONFIG.includes(extension))
        return 'config';
    if (FILE_CATEGORIES.DOCS.includes(extension))
        return 'docs';
    if (FILE_CATEGORIES.ASSETS.includes(extension))
        return 'assets';
    if (FILE_CATEGORIES.DATA.includes(extension))
        return 'data';
    return 'other';
}
/**
 ...
 */
export function createBackupFilename(originalPath, timestamp) {
 ...
 */
export function shouldAutoApprove(filePath, operationType) {
 ...
 */
export function parseOperationType(input) {
 ...
 */
export function estimateOperationComplexity(targets, operationType) {
    const factors = [];
