...
import { RISK_CONFIG, FILE_SIZE_THRESHOLDS } from './safety-constants.js';
import { isSystemFile, isConfigFile, isSecurityFile } from './safety-utils.js';
export class RiskAssessmentEngine {
    riskFactorWeights = new Map();
    safetyThresholds = new Map();
    constructor() {
     ...
     */
    async identifyRiskFactors(operation, targets) {
        const factors = [];
        ...
        const largeFiles = targets.filter(t => t.size && t.size > FILE_SIZE_THRESHOLDS.LARGE_FILE);
     ...
     */
    determineSafetyLevel(operation, targets) {
        if (operation.type === 'delete')
            return 'dangerous';
        if (operation.type === 'move')
            return 'risky';
        if (targets.some(t => isSystemFile(t.path)))
            return 'risky';
        if (targets.some(t => isConfigFile(t.path)))
            return 'cautious';
        return 'safe';
    }
    /**
     ...
     */
    determineImpactLevel(operation, targets) {
        if (operation.type === 'delete' || targets.length > RISK_CONFIG.BULK_OPERATION_THRESHOLD)
            return 'major';
        if (operation.type === 'move' || targets.length > 5)
            return 'significant';
        if (targets.length > 2)
            return 'moderate';
        return 'minimal';
     ...
     */
    calculateConfidence(riskFactors, targets) {
     ...
     */
    generateReasoning(riskFactors, riskLevel) {
     ...
     */
    canAutoApprove(riskLevel, riskFactors) {
        ...
        return !riskFactors.some(f => blockingFactors.includes(f.type) && f.severity === 'high');
    }
    /**
     * Create conservative assessment when normal assessment fails
     */
    createConservativeAssessment(operation, targets) {
        return {
            level: 'high',
