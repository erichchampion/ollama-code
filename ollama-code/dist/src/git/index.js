...
Format as JSON array of CommitSuggestion objects.`;
     ...
     */
    generateFallbackCommitSuggestions(stagedFiles, diff) {
        const suggestions = [];
     ...
     */
    formatCommitMessage(suggestion) {
     ...
     */
    async generatePRDescription(targetBranch = 'main') {
        const spinner = createSpinner('Generating PR description...');
        spinner.start();
        try {
            // Get commits since target branch
            const { stdout: commitsOut } = await execAsync(`git log ${targetBranch}..HEAD --oneline --format="%s"`, { cwd: this.workingDir });
            // Get file changes
            const { stdout: filesOut } = await execAsync(`git diff ${targetBranch}..HEAD --name-only`, { cwd: this.workingDir });
            // Get detailed diff stats
            const { stdout: statsOut } = await execAsync(`git diff ${targetBranch}..HEAD --stat`, { cwd: this.workingDir });
