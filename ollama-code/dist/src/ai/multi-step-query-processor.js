...
export class MultiStepQueryProcessor {
    ...
    constructor(aiClient, projectContext, conversationManager) {
     ...
     */
    extractSemanticContext(query, previousQueries) {
     ...
     */
    buildProcessingPrompt(queryInfo) {
        ...
        if (semantic.topics.length > 0 || semantic.entities.length > 0) {
            ...
            if (semantic.topics.length > 0)
                prompt += `- Topics: ${semantic.topics.join(', ')}\n`;
            if (semantic.entities.length > 0)
                prompt += `- Entities: ${semantic.entities.join(', ')}\n`;
            if (semantic.relationships.length > 0)
                prompt += `- Actions: ${semantic.relationships.join(', ')}\n`;
            prompt += '\n';
        }
        prompt += `**Instructions:**\n`;
     ...
     */
    generateSuggestions(queryInfo, result) {
        const suggestions = [];
     ...
     */
    detectFollowUpNeeds(query, result) {
        // Check query complexity indicators
        const complexityIndicators = [
            'analyze', 'review', 'explain', 'understand', 'explore', 'comprehensive'
        ];
        const hasComplexityIndicator = complexityIndicators.some(indicator => query.toLowerCase().includes(indicator));
     ...
     */
    requiresRefinement(query, result) {
...
//# sourceMappingURL=multi-step-query-processor.js.map