...
export class CIPipelineIntegrator {
     ...
     */
    async runArchitecturalAnalysis() {
    ...
    async runRegressionAnalysis() {
        ...
        return {
            overallRisk: regressionResults.riskAssessment.overallRisk,
            ...
            riskFactors: regressionResults.riskAssessment.riskFactors?.map((f) => f.description) || [],
            mitigation: regressionResults.recommendations.map((r) => r.action)
        };
    }
    /**
    ...
    async runQualityAnalysis() {
        ...
        return {
            ...
            testCoverage: typeof qualitySnapshot.metrics.testing.coverage === 'object' ?
                qualitySnapshot.metrics.testing.coverage.percentage || 0 :
                qualitySnapshot.metrics.testing.coverage,
            documentation: qualitySnapshot.metrics.maintainability.documentation || 0,
            maintainability: qualitySnapshot.metrics.maintainability.overallScore || 80,
    ...
    calculateOverallScore(results) {
        ...
        if (results.security) {
            ...
            const securityScore = Math.max(0, 100 - (results.security.criticalVulnerabilities * 20 +
        ...
        if (results.performance) {
            ...
            const performanceScore = Math.max(0, 100 - (results.performance.criticalIssues * 15 +
     ...
     * Evaluate quality gates
     */
    async evaluateQualityGates(results, overallScore) {
        const gates = [];
        ...
        if (results.regression) {
            ...
            const regressionPassed = ((threshold === 'high' && regressionLevel !== 'high') ||
     ...
     */
    generateRecommendations(results, qualityGates) {
     ...
     */
    async generateReports(results, qualityGates, overallScore) {
        const reportUrls = [];
        try {
     ...
     * Generate markdown report
     */
    generateMarkdownReport(results, qualityGates, overallScore) {
     ...
     */
    generateJUnitReport(qualityGates) {
 ...
 */
export const DEFAULT_CI_PIPELINE_CONFIG = {
    platform: 'github',
...
//# sourceMappingURL=ci-pipeline-integrator.js.map