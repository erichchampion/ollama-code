...
export class PullRequestReviewer {
    ...
    async reviewPullRequest(prInfo) {
        ...
        try {
            ...
            const [codeFindings, securityFindings, performanceFindings, architectureFindings, testFindings] = await Promise.all([
     ...
     */
    async reviewDiff(diffText, metadata) {
     ...
     */
    async performCodeReview(prInfo) {
        const findings = [];
        try {
     ...
     */
    async performSecurityReview(prInfo) {
        const findings = [];
        try {
            const filesToAnalyze = prInfo.files
                .filter(f => f.status !== 'deleted')
                .map(f => path.join(this.config.repositoryPath, f.path));
            const securityResults = await this.securityAnalyzer.analyzeFile(filesToAnalyze[0] || '', 'low');
            securityResults.forEach((vuln) => {
                findings.push({
                    id: this.generateFindingId(),
                    type: 'security',
     ...
     */
    async performPerformanceReview(prInfo) {
        const findings = [];
        try {
            const filesToAnalyze = prInfo.files
                .filter(f => f.status !== 'deleted')
                .map(f => path.join(this.config.repositoryPath, f.path));
            const perfResults = await this.performanceAnalyzer.analyzeFile(filesToAnalyze[0] || '', { severityThreshold: 'low', analyzeComplexity: true, checkMemoryLeaks: true });
     ...
     */
    async performArchitectureReview(prInfo) {
        const findings = [];
        try {
            // Architecture analysis would integrate with architectural analyzer
            // For now, provide basic architectural feedback
            const structuralChanges = prInfo.files.filter(f => f.path.includes('src/') && (f.status === 'added' || f.status === 'deleted'));
     ...
     */
    async performTestReview(prInfo) {
        const findings = [];
        try {
            const sourceFiles = prInfo.files.filter(f => f.status !== 'deleted' &&
                !f.path.includes('.test.') &&
                !f.path.includes('.spec.') &&
                !f.path.includes('/test/') &&
                (f.path.endsWith('.ts') || f.path.endsWith('.js') || f.path.endsWith('.py')));
            const testFiles = prInfo.files.filter(f => f.path.includes('.test.') ||
                f.path.includes('.spec.') ||
                f.path.includes('/test/'));
     ...
     */
    calculateMetrics(prInfo, findings) {
        ...
        const testFiles = prInfo.files.filter(f => f.path.includes('.test.') || f.path.includes('.spec.')).length;
        const sourceFiles = prInfo.files.filter(f => !f.path.includes('.test.') && !f.path.includes('.spec.') &&
            (f.path.endsWith('.ts') || f.path.endsWith('.js'))).length;
     ...
     */
    generateRecommendation(findings, metrics) {
        ...
        const blockingIssues = findings.filter(f => f.severity === 'critical' ||
     ...
     */
    generateSummary(prInfo, findings, metrics) {
        const parts = [];
        // Overall assessment
     ...
     */
    generateSuggestions(prInfo, findings, metrics) {
        const suggestions = [];
        // Quality suggestions
...
//# sourceMappingURL=pull-request-reviewer.js.map