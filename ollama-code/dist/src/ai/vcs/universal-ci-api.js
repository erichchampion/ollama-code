...
export class UniversalCIAPI {
     ...
     */
    async detectPlatformViaIndicators() {
        try {
            for (const [platform, config] of Object.entries(PLATFORM_DETECTION)) {
     ...
     */
    async generateConfigurationTemplate() {
        if (!this.detectedPlatform) {
            await this.initialize();
        }
     ...
     */
    createCIPipelineConfig() {
        const optimized = this.optimizeForPlatform();
        return {
            repositoryPath: optimized.repositoryPath,
            platform: (this.detectedPlatform === 'custom' ? 'github' : this.detectedPlatform),
            ...
            reportFormat: (optimized.reportFormat || 'json'),
...
//# sourceMappingURL=universal-ci-api.js.map