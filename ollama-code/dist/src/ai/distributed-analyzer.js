...
export class DistributedAnalyzer extends EventEmitter {
     ...
     */
    async analyzeInParallel(chunks) {
    ...
    async processChunksInParallel(chunks) {
        return new Promise((resolve, reject) => {
            ...
            const processNextChunk = (workerId) => {
            ...
            const onChunkError = (chunkId, error) => {
    ...
    assignChunkToWorker(workerId, chunk) {
    ...
    resolveEdgeConflict(existing, incoming, chunkId) {
    ...
    analyzeDependencies(files, projectContext) {
    ...
    optimizeChunkDistribution(chunks) {
    ...
    updatePerformanceMetrics(startTime, totalChunks, completedChunks) {
 ...
 */
async function analyzeChunkInWorker(chunk, config) {
    const startTime = Date.now();
...
//# sourceMappingURL=distributed-analyzer.js.map