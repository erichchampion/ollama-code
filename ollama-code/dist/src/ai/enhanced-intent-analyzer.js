...
export class EnhancedIntentAnalyzer {
    aiClient;
    responseCache = new LRUCache(100);
    patternRules = [];
     ...
     */
    async aiAnalysisWithTimeout(input, context, timeout) {
        if (!this.aiClient) {
     ...
     */
    async templateBasedAnalysis(input, context) {
        const templates = [
            {
     ...
     */
    async minimalistFallback(input) {
        return {
            type: 'conversation',
            action: input,
            entities: this.extractAllEntities(input),
    ...
    assessRiskLevel(type, complexity) {
        if (type === 'command' && complexity === 'complex')
            return 'high';
        if (type === 'task_request' && complexity === 'expert')
            return 'high';
...
Return only valid JSON, no explanatory text.`;
    ...
    createEmergencyFallback(input, lastError) {
...
//# sourceMappingURL=enhanced-intent-analyzer.js.map