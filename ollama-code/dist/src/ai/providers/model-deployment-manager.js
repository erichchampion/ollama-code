...
import { DEPLOYMENT_CONFIG } from './config/advanced-features-config.js';
...
export class ModelDeploymentManager extends EventEmitter {
     ...
     */
    async registerModel(name, version, type, modelPath, metadata = {}) {
     ...
     */
    async startInstance(deploymentId) {
        const deployment = this.registry.deployments.get(deploymentId);
        if (!deployment) {
            throw new Error(`Deployment not found: ${deploymentId}`);
        }
    ...
    async ensureDirectories() {
        await DirectoryManager.ensureDirectories(this.workspaceDir, this.modelsDir, this.logsDir);
    }
    async loadRegistry() {
    ...
    mergeWithDefaults(config, model) {
        const baseDefaults = {
    ...
    async createLoadBalancer(deploymentId, scaling) {
        return {
    ...
    async startModelProcess(instance, deployment) {
    ...
    selectLeastConnectedInstance(instanceIds) {
    ...
    selectWeightedInstance(instanceIds, loadBalancer) {
...
class PortAllocator {
    ...
    minPort;
    maxPort;
    constructor(minPort = DEPLOYMENT_CONFIG.ports.minPort, maxPort = DEPLOYMENT_CONFIG.ports.maxPort) {
...
//# sourceMappingURL=model-deployment-manager.js.map