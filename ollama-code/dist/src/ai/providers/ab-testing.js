...
export class ABTestingFramework extends EventEmitter {
     ...
     */
    performTTest(controlValues, testValues, _confidenceLevel) {
     ...
     */
    calculateCohenD(controlValues, testValues) {
        const controlMean = controlValues.reduce((sum, val) => sum + val, 0) / controlValues.length;
        const testMean = testValues.reduce((sum, val) => sum + val, 0) / testValues.length;
    ...
    getActiveTests() {
        const now = new Date();
        return Array.from(this.tests.values()).filter(test => test.enabled && now >= test.startDate && now <= test.endDate);
    }
    /**
     * Cancel a test
     */
