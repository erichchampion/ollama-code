...
export class ResponseFusionEngine extends EventEmitter {
     ...
     */
    async fusedComplete(prompt, options = {}, fusionOptions = {}) {
        ...
        try {
            ...
            const qualifiedResponses = this.filterAndWeightResponses(providerResponses, validationResults, strategy);
            // Check if we have minimum required responses
            if (qualifiedResponses.length < request.requireMinimum) {
                throw new Error(`Insufficient qualified responses: got ${qualifiedResponses.length}, required ${request.requireMinimum}`);
            }
            // Synthesize final response
            const fusedResponse = await this.synthesisEngine.synthesize(qualifiedResponses, strategy, fusionOptions.synthesisConfig);
            // Generate analysis
    ...
    async executeParallelRequests(request) {
        const promises = request.providers.map(async (providerId) => {
    ...
    filterAndWeightResponses(responses, validationResults, strategy) {
    ...
    analyzeResponses(responses, fusedResponse) {
    ...
    generateCacheKey(prompt, options, strategy) {
    ...
    calculateQualityScore(response, provider) {
    ...
    calculateConsensusWeight(response, allResponses) {
        // Calculate weight based on agreement with other responses
        return 0.8; // Placeholder
    }
    calculateDynamicWeight(response, allResponses, validationResults) {
        const validation = validationResults.get(response.providerId);
    ...
    identifyConflictPoints(responses) {
        const conflicts = [];
    ...
    identifyProviderStrengths(responses) {
        const strengths = {};
        for (const response of responses) {
    ...
    generateFollowupSuggestions(responses, fusedResponse) {
    ...
    areStatementsConflicting(stmt1, stmt2) {
    ...
    calculateNumericalVariance(numberSets) {
...
class ResponseValidationEngine {
    ...
    hasLogicalInconsistencies(content) {
        ...
        if (numbers && numbers.length > 1) {
            ...
            const hasConflictingRanges = values.some((val, i) => values.slice(i + 1).some(otherVal => Math.abs(val - otherVal) / Math.max(val, otherVal) > FUSION_CONFIG.validation.varianceThreshold));
            if (hasConflictingRanges)
                return true;
        }
...
class ResponseSynthesisEngine {
    async synthesize(responses, strategy, config = {}) {
    ...
    calculateSynthesisQuality(responses, content) {
        if (responses.length === 0)
            return 0;
    ...
    calculateDiversityScore(responses) {
        if (responses.length < 2)
            return 0.0;
    ...
    calculateReliabilityScore(responses) {
        if (responses.length === 0)
            return 0.0;
...
//# sourceMappingURL=response-fusion.js.map