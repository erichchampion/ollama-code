...
export class ProviderDashboard extends EventEmitter {
    ...
    setupEventListeners() {
        ...
        this.eventHandlers.budgetLimitReached = (data) => {
            this.createAlert(data.id, 'budget_exceeded', 'error', `Budget limit reached: ${data.type} (${data.usage}/${data.limit})`);
        };
        this.eventHandlers.budgetThresholdReached = (data) => {
            this.createAlert(data.id, 'budget_exceeded', 'warning', `Budget threshold reached: ${data.percentage.toFixed(1)}% of ${data.type}`);
        };
        this.eventHandlers.healthCheckFailed = (data) => {
            this.createAlert(data.id, 'provider_down', 'critical', `Provider health check failed: ${data.error}`);
        };
     ...
     * Create a new alert
     */
    createAlert(providerId, type, severity, message) {
    ...
    checkAlerts(providerId) {
        ...
        if (this.config.alertThresholds && metrics.errorRate > this.config.alertThresholds.errorRate) {
            this.createAlert(providerId, 'error_rate', 'warning', `High error rate: ${(metrics.errorRate * 100).toFixed(1)}%`);
        }
        // Check response time
        if (this.config.alertThresholds && metrics.responseTime.avg > this.config.alertThresholds.responseTime) {
            this.createAlert(providerId, 'response_time', 'warning', `High response time: ${metrics.responseTime.avg.toFixed(0)}ms`);
        }
    }
    /**
    ...
    getCostAnalysis() {
        ...
        for (const providerId of providerIds) {
            ...
            const costTrend = 'stable'; // Would calculate from historical snapshots
            analyses.push({
                providerId,
                dailyCost,
                monthlyCost,
     ...
     */
    getProviderAlerts(providerId) {
        return Array.from(this.alerts.values()).filter(alert => alert.providerId === providerId && !alert.acknowledged);
    }
    /**
...
//# sourceMappingURL=provider-dashboard.js.map