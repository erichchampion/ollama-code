 ...
 */
export async function readFileLines(filePath, start, end, encoding = 'utf-8') {
    try {
 ...
 */
export async function appendTextFile(filePath, content, options = {}) {
 ...
 */
export async function findFiles(directory, options = {}) {
 ...
 */
export async function streamFile(sourcePath, destPath, options = {}) {
    const { overwrite = false, createDir = true } = options;
    if (!isValidFilePath(sourcePath) || !isValidFilePath(destPath)) {
        throw createUserError(`Invalid file path: ${!isValidFilePath(sourcePath) ? sourcePath : destPath}`, {
            category: ErrorCategory.VALIDATION,
 ...
 */
export async function createTempFile(options = {}) {
